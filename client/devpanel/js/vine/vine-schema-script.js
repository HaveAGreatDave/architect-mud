...
const _scriptNodeDefs = {
  ...
  spawn: {
    ...
    renderProperties: (n, ed, id) => `
      ${_helpBox(id,
        'Puts something in the world: an enemy instance from an enemies template, or an item into a zone. Blank zone means the actor\'s room. Leave "announce" empty for the stock arrival line; type one to override it. An enemy with announce set to the literal word false arrives SILENTLY — for a tail the player hasn\'t noticed yet.\n\nDEAD DROP: give an item spawn a container (a furniture id, or a name to match in that zone) and the item goes INSIDE it rather than onto the open floor — really there, really retrievable, but not visible to the next person through the room. If the container can\'t be found the drop is skipped, never dumped on the floor.\n\nFLAGS (enemies only) are stamped onto the spawned instance, not the template — so two spawns from one template can hunt two different people. ${actor.id} resolves to the player who tripped the script, which is what a CHASE node in quarry:flag mode reads. Pair it with leash_radius:-1 for a pursuer that doesn\'t give up at the district line.\n\nDo NOT set flags.hunter to make something chase: hunter is the police/enforcement EXEMPTION from the sanctuary and enemy_barrier rules, and a mob carrying it\'ll walk into a safe room after the player.',
        'kind: enemy\nid: enemy_halcyon_adjuster\nannounce: false\nflags: { "suspect_id": "${actor.id}", "leash_radius": -1 }\n\n→ it arrives quietly, and it\'s looking for one specific person'
      )}
