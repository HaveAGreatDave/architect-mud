...
async function zoneWindowDelete(id, name, zoneId) {
  ...
  const r = await API('/staging/stage', 'POST', {
    ...
    requestBody: {}, description: `Delete window ${name} (${id})`,
  });
  if (r?.error) { toast(r.error, true); return; }
  toast(`Window "${name}" staged for deletion — publish to apply`);
  await updateStagingBadge();
...
function _openZoneWindowModal(w, zoneId) {
  ...
  document.getElementById('modal-body').innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <div class="field"><label>Object ID <span style="color:var(--text-dim);font-weight:400">(auto)</span></label>
        <input id="zwm-id" value="${w.id||''}" placeholder="auto (window1, window2…)"></div>
      <div class="field"><label>Display Handle <span style="color:var(--text-dim);font-weight:400">(players use this)</span></label>
        <input id="zwm-handle" value="${w.handle||''}" placeholder="auto from name"></div>
    </div>
    <div class="field"><label>Name</label><input id="zwm-name" value="${w.name||'window'}" oninput="
      const h=document.getElementById('zwm-handle');
      if(!h._touched) h.value=this.value.toLowerCase().replace(/\\s+/g,'_').replace(/[^a-z0-9_]/g,'');
    "></div>
  ...
  // Mark handle as manually touched if user edits it directly
  document.getElementById('zwm-handle').addEventListener('input', function() { this._touched = true; });
  document.getElementById('modal-save').onclick = async () => {
    const name = document.getElementById('zwm-name').value.trim();
    const handle = document.getElementById('zwm-handle').value.trim() || undefined;
    const customId = document.getElementById('zwm-id').value.trim() || undefined;
    if (!name) { toast('Name is required', true); return; }
    const body = {
      ...(customId && isNew ? { id: customId } : {}),
      name, handle,
    ...
    if (r?.error) { toast(r.error, true); return; }
    toast(isNew ? 'Window staged — publish to apply' : 'Window update staged — publish to apply');
    closeModal();
    await updateStagingBadge();
    zoneWindowsRefresh(zoneId);
  };
