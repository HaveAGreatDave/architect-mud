...
(function (global) {
  const TAG_CATALOG = {
    ...
    truck_depot: { label: 'Truck Depot', shape: 'object', scope: 'zone', group: 'Zone: Flight',
      help: 'A freight yard (THE LONG HAUL). `drive` issues a rig here, `haul` shows the load board, and a delivery only pays when the truck is standing in the depot the load names. Put it on the DRIVABLE STREET TILE outside a freight building, never on the facade itself — a facade carries building_type and is therefore solid, so a truck would collide with it rather than park in it. Shape: { "name": "Kessler Street Yard" }. Depots are also each other\'s destinations: the load board at one offers runs to every other depot it knows about, and a board with nowhere to send you is empty.' },
