...
(function (global) {
  const TAG_CATALOG = {
    ...
    no_repair: { label: "Can't Be Repaired", shape: 'flag', scope: 'class', group: 'Core',
      help: "Opt-OUT of repair. Anything that wears (weapon / armour / body-slot apparel / tool) is repairable by default and needs no tag — use this only for the rare thing that genuinely can't be fixed at a bench (sealed chrome, burned firmware). See docs/systems-durability.md." },
    ...
    soap: { label: 'Soap', shape: 'flag', scope: 'class', group: 'Core',
      help: "Carried soap. Makes a wash at a sink or in open water count as a FULL clean (resets the hygiene clock rather than just rinsing what's on you)." },
    ...
    consumable_cleaner: { label: 'Consumable Cleaner', shape: 'flag', scope: 'class', group: 'Core',
      help: "A bottle of something that cleans and is USED UP doing it (acetone, bleach). Cleans a floor like a Cleaning Tool, but spends a unit per use — so the resolver reaches for a mop or brush FIRST and only falls back to this when there's no reusable tool to hand. Pair with cleaning_tool on the same item." },
    ...
    vermin_part: { label: 'Vermin Part', shape: 'flag', scope: 'class', group: 'Type',
      help: "Proof of a pest killed — a carapace, a tail. An NPC flagged `bounty_buyer` pays the specialist rate (0.7 of value) for these, the way `drug_buyer` and `food_buyer` do for theirs; everyone else pays the ordinary 0.4. It's the tag, not the item id, that the price hangs on, so the next pest anybody writes is already worth something to the right person." },
    ...
    layer: { label: 'Layer', shape: 'enum', scope: 'class', group: 'Equipment',
      ...
      help: "Which of the three worn layers this piece occupies on a body slot (underwear < outerwear < armor, innermost to outermost). Only one item per slot+layer; others see only your outermost layer. Also the order RAIN meets your clothes — an outer layer shields what's under it until it saturates. Ignored for weapon_hand and accessory. Defaults to outerwear if unset." },
    ...
    gets_wet: { label: 'Gets Wet', shape: 'flag', scope: 'class', group: 'Equipment',
      help: "Absorbs water (0–100) when worn in rain or snow. Wet clothing lowers cooling thresholds and speeds heat loss. Water runs outside-in through the `layer` stack, so how much reaches this piece depends on what's worn over it — and a garment WITHOUT this tag (plate, plastic) holds no water and sheds it onto whatever is underneath." },
    ...
    waterproof: { label: 'Waterproof', shape: 'flag', scope: 'class', group: 'Equipment',
      help: 'Sheds falling liquid. Two effects. RAIN: passes only 5% of the water that lands on it down to the layer beneath, so the garments (and skin) under it stay dry — the piece itself never gets wet. ACID: shields the slots this garment actually occupies (its own slot plus anything in `covers`) from the acid-rain burn, and is exempt from acid corrosion. Unlike `sealed`, acid protection is COVERAGE-based — head, torso, legs and feet must all be covered for full immunity; partial cover only scales the burn down.' },
    hydrophobic: { label: 'Insulates Wet', shape: 'flag', scope: 'class', group: 'Equipment',
      help: 'This garment keeps its `insulation` when soaked. Wool, neoprene, sealed shells and wicking synthetics do; cotton, denim and padding do not, and lose most of their value wet — which is why "cotton kills". Without this tag a soaked garment contributes almost nothing against the cold, so a rained-on hoodie is genuinely dangerous and a wet wool sweater is not. Also makes the piece less stifling in the heat, since the same water isn\'t being trapped against you.' },
    windproof: { label: 'Windproof (Shell)', shape: 'flag', scope: 'class', group: 'Equipment',
      help: "A sealed outer face the wind can't get through. Gives back the WIND CHILL share of the feels-like temperature for the slots this garment covers — torso is worth about two thirds of it, legs the rest — so a shell is what makes a gale survivable rather than merely a warmer coat. Does nothing about the actual air temperature: windproof isn't insulation, and a windproof shell over nothing still leaves you in the cold." },
    ...
    shielded: { label: 'EMP Shielded', shape: 'flag', scope: 'class', group: 'Equipment',
      help: "Faraday shielding. The item itself is immune to an EMP pulse, and — if it's also a container — so is everything inside it." },
    ...
    damage_type: { label: 'Damage Type', shape: 'enum', scope: 'class', group: 'Combat',
      ...
      help: "Physical damage category. Used to index per-part armor soak on defender. NOTE on `chemical`: it's a SPECIALIST type. Almost nothing soaks it, so it walks through armour — but it's multiplied down to ~30% against any target that isn't flagged `vermin` (see `typeEffectiveness` in combat.js), so a fumigant is devastating on roaches and feeble on people. Opt an enemy in with `flags.vermin: true`; the resistance is the default so a new enemy is never a silent hole." },
    ...
    arc_chain: { label: 'Arc Chain', shape: 'object', scope: 'class', group: 'Combat',
      help: 'A discharge weapon whose landed hit JUMPS to other hostiles in the room: { "jumps": 2, "falloff": 0.55 }. Each jump carries `falloff` of the previous hit and is resolved through the ordinary strike path, so every link rolls its own body part and is soaked by whatever armour covers it. Resolved INSIDE the swing (see the ARC CHAIN note in combat.js) — the same blow spreading, exactly like `spread` and `water_shock`, so there\'s no second to-hit roll and no extra cooldown. Two limits are deliberate: it reaches ENEMIES only (jumping to a person is an assault nobody aimed) and it does NOT carry `status_chance`, or a chain weapon would be a crowd-control button wearing a weapon\'s clothes. Clamped to 6 jumps and a falloff below 1.' },
    ammo_capacity: { label: 'Ammunition Capacity', shape: 'number', scope: 'class', group: 'Combat',
      help: 'This weapon spends ammunition: how many shots a full magazine/tank/cell holds. One is spent per SWING, hit or miss, and a dry weapon refuses BEFORE the swing so being empty costs the shot rather than the shot and the cooldown behind it. The live count is per-unit instance state in custom_data.ammo, so the weapon must also be `unique`; an ABSENT count reads as FULL, so a weapon that arrives by any route (bought, looted, granted) is loaded. Nothing authored before this tag existed carries it, so every other weapon in the game still fires forever. Pair with `ammo_type` to make it reloadable.' },
    ammo_type: { label: 'Ammunition Type', shape: 'text', scope: 'class', group: 'Combat',
      help: 'What this weapon eats, e.g. "pyre_fuel". RELOAD matches it against a carried item\'s `ammo_load.type` — by TYPE, never by item id, so a second weapon can be authored against the same ammunition without either one being edited. A weapon with `ammo_capacity` but no `ammo_type` can be fired dry and never refilled, which is a legitimate authoring choice for something disposable.' },
    ammo_load: { label: 'Ammunition Load', shape: 'object', scope: 'class', group: 'Combat',
      help: 'Marks an item as ammunition and says what it fits: { "type": "pyre_fuel", "units": 40 }. Consumed WHOLE by RELOAD — a partial top-up still spends the entire magazine, so reloading a half-full weapon wastes the difference. The `battery` tag is the flashlight\'s equivalent and is deliberately separate, so a torch cell can never feed a gun.' },
    dry_message: { label: 'Dry-Fire Message', shape: 'text', scope: 'class', group: 'Combat',
      help: 'The sentence printed when this weapon is fired empty. Optional — the fallback is "It clicks, and nothing happens." Worth authoring: a dry flame projector coughs an unlit mist, and that reads as a different failure from a dry pistol.' },
    reload_message: { label: 'Reload Message', shape: 'text', scope: 'class', group: 'Combat',
      help: 'The sentence printed on a successful RELOAD. Optional — the fallback names the weapon generically.' },
    ...
    cooking_medium: { label: 'Cooking Medium', shape: 'flag', scope: 'class', group: 'Cooking',
      help: 'This is in the pan to CARRY heat, not to be eaten — the water pasta boils in. It still counts as a liquid for everything that asks whether the pan is wet (fond, the sound it makes, the dry-starch gate), and counts for nothing at all when the dish is scored or named: it takes no cook session, never burns, never satisfies a recipe asking for stock, and is tipped away at `drain` or consumed silently at `plate`. Only meaningful alongside food_profile "liquid".' },
    ...
    pour_units: { label: 'Pours', shape: 'number', scope: 'class', group: 'Drinks',
      help: "How many 25ml measures one of these rows is. A 700ml bottle is 14; a can of tonic is 2 or 3. This is the unit every drink recipe counts in, and it's also what the recipe card converts back into millilitres. Absent reads as 1, so nothing silently vanishes from a build." },
    ...
    caffeine_mg: { label: 'Caffeine (mg/pour)', shape: 'number', scope: 'class', group: 'Drinks',
      help: 'Milligrams of caffeine in ONE POUR of this ingredient. Almost nothing needs it: caffeine is a fact about the CLASS, so coffee_base and tea_base carry a default (plugins/drinks/caffeine.js) and everything else carries none. Author it only where an item disagrees with its own class, which is the case that matters and goes BOTH ways — chicory sits in coffee_base with none in it, herbal tea sits in tea_base with none in it, and a cola is caffeinated while every other mixer on the shelf is not. A drink derives its whole kick from this and lands on the ordinary drug_coffee path, so a brewed coffee keeps you awake exactly the way a bought tin of it does. Under 20mg in the finished vessel applies no drug at all.' },
    drinkware: { label: 'Drinkware', shape: 'flag', scope: 'class', group: 'Drinks',
      help: "A REUSABLE drinking vessel — a mug, a glass, a shaker. You mix or brew into it, carry it, drink it down and rinse it out; it's never consumed. Requires the Fillable tag, whose capacity doubles as how many SERVINGS it holds. Not the same thing as a cooking `vessel`." },
    ...
    fragile: { label: 'Fragile', shape: 'flag', scope: 'class', group: 'Drinks',
      help: "Glass. AUTHORED BUT NOT YET READ: there's no drop/break seam to hang it off (player_inventory.condition exists but nothing decrements it for a dropped cup), so this is a marker for a future pass rather than live behaviour. Tag glassware with it now so nothing needs backfilling later." },
    dishware: { label: 'Dishware', shape: 'flag', scope: 'class', group: 'Drinks',
      help: "A plate, platter or strainer — kitchen kit that's neither a cooking vessel nor drinkware. It has no mechanics of its own; it exists to be owned, stored in a dish cabinet and set on a table. Deliberately NOT tagged `vessel`, so nobody can put a dinner plate on the stove and call it cooking." },
    ...
    edible_vessel: { label: 'Edible Vessel', shape: 'flag', scope: 'class', group: 'Cooking',
      help: "This vessel is EATEN as part of what it makes, instead of surviving the meal like a pan does. Bread is the only one: a sandwich isn't fillings served in a bread container, it IS the bread. The vessel is scored as an ingredient, lends its noun to the dish name, and is consumed by `plate`. Only meaningful alongside `vessel`." },
    ...
    heat_retention: { label: 'Heat Retention', shape: 'number', scope: 'class', group: 'Cooking',
      help: "How well a vessel holds heat, 0..1 (thin steel ~0.4, heavy cast ~0.8). Widens the peak window itself, so there's more time to plate it right. Only meaningful alongside `vessel`." },
    utensil: { label: 'Utensil', shape: 'flag', scope: 'class', group: 'Cooking',
      help: 'A hand tool for kitchen work — spoon, spatula, tongs, whisk, ladle. Purely a CATEGORY: nothing branches on it, the actual gates are can_stir/can_turn/can_chop. It exists so bulk commands can sweep by category ("drop all utensils", "put all utensils in the drawer") without naming every one. Tag the kitchen tool, not the weapon — a combat knife isn\'t a utensil even though it chops.' },
    ...
    storage_tier: { label: 'Storage Tier', shape: 'enum', scope: 'class', group: 'Preservation',
      options: ['dry', 'refrigerated', 'frozen'],
      help: 'OVERRIDE for which shelf this belongs on — the section a shop or a container groups it under (Dry Goods / Refrigerated / Frozen). Purely presentational: it decides nothing about spoilage, which is perishable + spoil_rate\'s job. Rarely needed, because the tier is derived from those two tags already; set it only where the derivation is wrong. FROZEN is the case that always needs it — nothing in an item\'s tags can tell you a fish fillet is sold frozen rather than fresh, so a freezer case has to be authored. See docs/reference/item-facets.md.' },
    preservative: { label: 'Preservative', shape: 'flag', scope: 'class', group: 'Preservation',
      help: 'A carried antioxidant/curing chemical the `preserve` verb spends to slow ONE perishable item\'s decay wherever it sits (a vial of BHT). Stacks with refrigeration rather than replacing it, and is refused on food that\'s already spoiling — it slows rot, it never reverses it.' },
    ...
    hack_device: { label: 'Hacking Device', shape: 'flag', scope: 'class', group: 'Gear',
      help: "Marks this item as an intrusion deck. Carrying one (in hand, not in a container) is the tool gate for HACK on hololocked doors and for jacking ATM terminals; it's also classed as contraband when jailed. Pair with the Unique tag so each deck keeps its own condition — failed breaches damage it." },
    hack_penalty: { label: 'Hack Penalty', shape: 'number', scope: 'class', group: 'Gear',
      help: 'On a Hacking Device: how much this deck ADDS to every target\'s Hack Difficulty. 0 (or unset) is a clean professional deck. 2 is junk — every hololock, ATM and safe reads two points harder while this is the best deck you\'re carrying. Only the best deck in your inventory is consulted.' },
    ...
    jammer_cell: { label: 'Jammer Cell', shape: 'flag', scope: 'class', group: 'Gear',
      help: 'A single-use power cell for a carried jammer. `jammer on` spends one and the field runs about four minutes. Deliberately a consumable rather than a recharge: a jammer you can leave running is a state you switch on in the morning, and the cost is what keeps it a decision.' },
    emp_charge: { label: 'EMP Charge', shape: 'flag', scope: 'class', group: 'Gear',
      help: "A single-use electromagnetic charge. `emp` fires the SAME pulse the ion storm does, scoped to your room — every unshielded `electronic` item carried by anyone present is `fried` until a bench repair, faraday containers spare their contents, and installed chrome blacks out. It doesn't care whose pockets it's in, including yours." },
    explosive_charge: { label: 'Breaching Charge', shape: 'flag', scope: 'class', group: 'Gear',
      help: "A single-use demolition charge. `breach <thing> with <charge>` opens the arming board, spends one, and leaves a fuse counting down on that object — after which anybody in the room, including you, is standing next to it. Deliberately NOT a weapon: there's no verb that throws one at a person, and the blast doesn't care whose it was." },
    demolishable: { label: 'Demolishable', shape: 'flag', scope: 'furniture', group: 'Furniture',
      help: "This object can have a charge wired to it. A builder decides what's worth blowing up when they place it, which is why `breach` reads a tag rather than a list of furniture ids. A detonation marks the row `destroyed` rather than deleting it, so the world keeps the scar." },
    rig_difficulty: { label: 'Rig Difficulty', shape: 'number', scope: 'furniture', group: 'Furniture',
      help: 'On a Demolishable object: how hard the arming board is, 1–10. Unset means 5. This scales the SEATING game only — a hardened target is fiddly to wire, never impossible, because a difficulty that could refuse outright would be a locked door wearing a minigame.' },
    null_device: { label: 'Null Gear', shape: 'flag', scope: 'class', group: 'Gear',
      help: 'Marks this item as Nullcraft hardware — a jammer, a scrambler, an intrusion platform. Carrying one (in hand, not in a container) is the tool gate for the Null operations that need hardware. Distinct from Hacking Device on purpose: a deck gets you INTO a thing, Null gear attacks what a thing depends on, and a player may sensibly carry both.' },
    null_intrusion: { label: 'Null Intrusion Strength', shape: 'number', scope: 'class', group: 'Gear',
      help: 'On Null Gear: how many points of the target\'s security this platform cancels. Subtracted from the effective difficulty of every Null operation. 0 (or unset) is a bare-hands rig that helps not at all; 6 is a Black Box. Only the best Null device you\'re carrying is consulted.' },
    null_jam_strength: { label: 'Jam Strength', shape: 'number', scope: 'class', group: 'Gear',
      help: 'On Null Gear: how hard this jammer pushes, 0–100. Merged with any PLANTED jammer already in the zone — surveillance keeps ownership of the planted half and this is only the carried contribution, so the two can never disagree about whether a room is jammed. Needs a Jammer Cell to run.' },
    null_jam_radius: { label: 'Jam Radius', shape: 'number', scope: 'class', group: 'Gear',
      help: "On Null Gear: how many tiles out the jam field reaches. 0 (or unset) means the room you're standing in and nothing else." },
    null_stealth: { label: 'Null Stealth', shape: 'number', scope: 'class', group: 'Gear',
      help: 'On Null Gear: how much this rig suppresses the trace you accrue, as a percentage 0–100. Trace is the clock on how long you can stay inside a system, so this buys TIME rather than power — the difference between a smash-and-grab and a quiet afternoon.' },
    null_selective: { label: 'Selective Jamming', shape: 'flag', scope: 'class', group: 'Gear',
      help: 'On Null Gear: this jammer targets one signal instead of flooding the room. A selective jammer contributes NOTHING to the zone jam field — which is the point: your allies\' radios, your own deck and every friendly device keep working. Elite hardware; a blunt jammer is cheaper and louder.' },
    ...
    // bought, stolen, dropped and looted, and installing it consumes the row.
    // See docs/systems-augments.md.
    augment_hardware: { label: 'Augment Hardware', shape: 'text', scope: 'class', group: 'Hardware',
      help: 'This item IS a piece of uninstalled chrome; the value is the augment id it fits (e.g. aug_myomer_bundles). The install path finds the row by the augment\'s own item_id, so this tag is for shelves, stock filters and examine — not for the gate.' },
    no_install: { label: "Can't Be Installed", shape: 'flag', scope: 'class', group: 'Hardware',
      help: "Wreckage. Marks chrome that death or a botched theatre destroyed — scrap value and repair stock, and nothing in the game will ever fit one to a body. Pair with Can't Be Repaired." },
    scrap: { label: 'Scrap', shape: 'flag', scope: 'class', group: 'Hardware',
      help: 'Worth its materials and nothing else. Sells, stacks on a shelf under salvage, and is never mistaken for the working version of the same thing.' },
    calibration_rig: { label: 'Calibration Rig', shape: 'flag', scope: 'class', group: 'Tools',
      help: "Consumed by one CALIBRATE. Better rigs make the tuning board easier, never higher-scoring — a fine instrument makes the job easier, it doesn't make the machine better than it is. Not needed at a clinic bench." },
    ...
    fuel_price_sign: { label: 'Fuel Price Board', shape: 'flag', scope: 'furniture', group: 'Capabilities',
      help: "Examining this renders a forecourt price board. It holds NO prices of its own — it prints whatever the systems that actually charge for fuel report through the `fuel.prices` gather hook for this zone (trucking answers with a tank of diesel wherever there's a pump; the fuelstation plugin answers with the per-unit gasoline in any Fuel Source furniture standing here). A board in a room where nothing is sold renders as dark, which is a correct answer rather than an error." },
    ...
    hull_patch: { label: 'Hull Patch', shape: 'flag', scope: 'class', group: 'Systems',
      help: 'Consumed by `fix` on a boat away from a bench: puts some hull back and is used up doing it. The same field-vs-bench split `durability.js` draws for gear — a patch is capped well short of sound and can never take a hull past that cap, because the thing that makes a bench worth walking to is that it is the only place a repair is finished properly. Owner: powerboat (yard.js).' },
    ...
    accommodates: { label: 'Accommodates Mutations', shape: 'list', scope: 'class', group: 'Systems',
      ...
      help: "Body shapes this garment is cut for. A mutation that blocks this slot is refused UNLESS the garment accommodates it (mutations plugin). Shapes, never mutation ids — a coat has a slit for a tail, it isn't made for one specific tail." },
    mutagen: { label: 'Mutagen', shape: 'flag', scope: 'class', group: 'Systems',
      help: 'Wildblood mutagen. Consuming it applies a mutagen mutation, and is REFUSED server-side for anyone without Inner Circle Wildblood rep AND the Quickening (mutations plugin).' },
    ...
    crop: { label: 'Legal Crop', shape: 'flag', scope: 'class', group: 'Systems',
      help: "Baled legal agricultural produce — the bottom rung of the Reach dead-drop ladder. Orderable like a raw but tagged NEITHER raw_drug NOR contraband, so it isn't a manufacturing felony and customs never scans it. Cures into its finished product via a fabrication recipe, not a chemistry cook." },
    ...
    no_spawn: { label: 'No Spawns', shape: 'flag', scope: 'zone', group: 'Zone: Law & Hazard',
      help: 'Suppress enemy spawns in this zone — and, since the leash work, refuse entry to any enemy that tries to WALK in (roaming, patrolling or chasing). Spawn suppression alone was never enough once mobs could move.' },
    enemy_barrier: { label: 'Enemy Barrier', shape: 'flag', scope: 'zone', group: 'Zone: Law & Hazard',
      help: 'Hard "mobs stop here" paint: no enemy may enter this tile, whatever it\'s doing. For the handful of tiles that really are a wall — the city side of the Curtain. This is the runtime backstop the Curtain never had; it holds today only because no exit link crosses it, which one mis-authored exit would undo. Use no_spawn for ordinary "keep mobs out of the camp" cases.' },
    emp_shadow: { label: 'EMP Shadow', shape: 'flag', scope: 'zone', group: 'Zone: Law & Hazard',
      help: "An ion storm does nothing to anyone standing here: the EMP pulse skips this tile, so carried electronics aren't fried and installed chrome isn't knocked out. For places that run on nothing an EMP can reach. NOTHING READS THIS YET — it's stamped across Deadwater ready for the jamming work, and the pulse handler in plugins/weather doesn't consult it until then. Note it's deliberately gated on the FLAG and never on a region id, so engine and plugin code never name content. Don't author any prose about it: in Deadwater the fact that nobody remarks on the storm is the point." },
    ...
    is_dreamzone: { label: 'Dreamzone', shape: 'flag', scope: 'zone', group: 'Zone: Structure', runtime: true,
      help: "RETIRED — don't use. Marked the old SHARED authored dreamzones, where two people on the same drug met each other inside the hallucination. Every dream and trip is instanced now (private, RAM-only rooms). Kept only so the login rescue still recognises a legacy row." },
    ...
    world_exit_zone: { label: 'World Exit Zone', shape: 'ref', refTable: 'zones', scope: 'zone', group: 'Zone: Structure',
      help: 'Exterior seam zone for this building — where OUT ultimately lands. TWO MEANINGS, by where it sits: on a FACADE it\'s the STREET tile out front (authored, geometry\'s business); on an INTERIOR tile it\'s the facade again, i.e. the map anchor — which the map owns and pushes, so change that one on the map, not the tile.' },
    ...
    window: { label: 'Window', shape: 'object', scope: 'zone', group: 'Zone: Structure',
      help: "This room has a window onto the outdoors: { name, description, light, visibility } — transmissions 0–1, default 0.8, and every key optional ({} is a plain window). It lets daylight in (the only ambient an interior gets), can be looked through, and its curtains can be drawn — and it's what decides whether you SEE an approaching storm or only feel it through the wall (skyVantage, engine/environment.js). Curtain and glass state are RUNTIME, held in RAM like a door lock: never author them here. Faces outdoors only; there's no window between two interior rooms." },
    ...
    liquid: { label: 'Liquid', shape: 'tristate', scope: 'zone', group: 'Zone: Properties', preset: true, presetFrom: 'terrain',
      help: "OVERRIDE. You're IN this tile rather than ON it — fishing casts into it, and the overland void rim doesn't exist here. Preset by terrain (water ⇒ liquid). Omit to inherit; set No to force a water tile solid." },
    ...
    underwater: { label: 'Underwater', shape: 'tristate', scope: 'zone', group: 'Zone: Properties', preset: true, presetFrom: 'terrain',
      help: 'OVERRIDE. A submerged tile below a surface water tile (link up/down): always submerged (a boat doesn\'t help), colder, and dark, and it starts a breath timer that drowns you. Preset by the "underwater" terrain, which paints identically to water — the difference is what it does to you, not what it looks like.' },
    ...
    marina_berths: { label: 'Marina Berths', shape: 'number', scope: 'zone', group: 'Zone: Structure',
      help: 'How many boats can lie alongside this pontoon. A tile carrying this IS a marina pontoon — there is deliberately no companion `marina: true`, because that would be a second statement of the same fact and a second thing to keep in step. Put it on the DECK you stand on, never on the open water beside it: a berth is a place a player walks to and boards from, so the tile also wants the pier set (`pier`, `liquid: false`, `swimmable: false`, `routable`) that makes a water tile standable. Not to be confused with `berth`, which is the cargo-ship animation and is the opposite arrangement — open water, no deck, nothing to stand on. Owner: powerboat (yard.js), which reads the count to decide how many `boats.berth_zone` rows may point here.' },

    boat_covered: { label: 'Covered Boat Bays', shape: 'number', scope: 'zone', group: 'Zone: Structure',
      help: 'How many hulls this INTERIOR room keeps under a roof, on cradles, with a hoist. The expensive tier and the only place `refit` finishes a hull properly — a patch anywhere else is capped well short of sound, which is the whole reason to pay for a shed. Author what is LETTABLE, not what the prose says is standing there: occupancy is counted against the `boats` rows, so bays filled by scenery would read as free. Owner: powerboat (yard.js).' },
    boat_hardstanding: { label: 'Boat Hardstanding', shape: 'number', scope: 'zone', group: 'Zone: Structure',
    ...
    airfield_id: { label: 'Airfield ID', shape: 'text', scope: 'zone', group: 'Zone: Flight',
      help: 'Which airfield this tile belongs to — the membership pointer into the `airfields` table, exactly like region_id. Everything ABOUT the field (its name, which desks it runs, what fuel it stocks, whether it\'s a helipad) lives on that row, not here. A tile with an id that has no row reads as "no field here".' },
    ...
    truck_depot: { label: 'Truck Depot', shape: 'object', scope: 'zone', group: 'Zone: Flight',
      help: 'A freight depot (THE LONG HAUL) — the SHOP, and it goes INSIDE a building you walk into, never on the street. It used to live on the street tile, which meant the dealer\'s line, the freight board and the commodities exchange all bloomed over the road because you crossed a particular kerb; nothing else in the game does that, and the hangar this system copies has been a walk-in interior since the day it was written. Put it on an interior zone (is_interior, behind a facade) and give it a `yard`: the id of the DRIVABLE street tile outside the roller door, which is where `drive` actually mounts the rig, because a building tile is solid and has no road under it. Shape: { "name": "The Roadhead Depot", "yard": "zone_terminus_1202_940" }. Depots are also each other\'s destinations: the load board at one offers runs to every other depot it knows about, and a board with nowhere to send you is empty.' },
    truck_bunkroom: { label: 'Truck Bunkroom', shape: 'flag', scope: 'zone', group: 'Zone: Flight',
      help: 'The room behind a depot office where a driver can stop the night (THE LONG HAUL). Four cots, a television, a fridge with something in it and a sink — every depot on the network has one and they\'re deliberately identical, because the point is that a driver who has been out for two days knows exactly what\'s waiting at the next one. Purely a LABEL: nothing gates on it, the cots are ordinary beds and the fridge is an ordinary container, so the room works whether or not anything ever reads this. It exists so that "where can somebody sleep on this network" has one answer to ask rather than five zone ids to remember.' },
    truck_yard: { label: 'Truck Yard (apron)', shape: 'text', scope: 'zone', group: 'Zone: Flight',
      help: 'The hardstand outside a depot door — the drivable tile a depot\'s `yard` points AT. Purely descriptive: it\'s what makes the street say there\'s a depot through that door, so a driver who parks on the apron can find the shop. The truth about which tile a depot uses lives in the depot\'s own `yard` key, never here, so a mismatch between the two is only ever a missing sentence and never a broken door. Value is the yard\'s spoken name.' },
    loading_dock: { label: 'Loading Dock', shape: 'object', scope: 'zone', group: 'Zone: Flight',
      help: 'A business that takes freight off a truck (THE LONG HAUL). Docks in a region are mixed into that region\'s freight boards, which is what makes the city a DESTINATION rather than the road to the map\'s edge — and what gives the fleet ladder a bottom rung, since a local run pays about a third of an in-region depot job and needs no crossing. Put it on the DRIVABLE STREET TILE outside the business, never on the facade itself (a facade carries building_type and is solid — a truck would hit it rather than back onto it). Delivery needed no new code: a contract already keys on a zone id. Shape: { "name": "Shelf Life" }.' },
    weigh_station: { label: 'Weigh Station', shape: 'object', scope: 'zone', group: 'Zone: Flight',
      help: 'A weighbridge a rig has to cross (THE LONG HAUL). It measures WEIGHT, NOT CONTRABAND: it compares the trailer against the manifest and against its plate rating, so a legal overweight load fails the same scale as a hidden one, and the counter-play is hauling less or declaring more. Put it on a DRIVABLE ROAD TILE that traffic has to pass — the point of the system is a decision taken before an inspection you can see coming, so a scale nobody drives over is decoration. It\'s skipped entirely in a region flagged lawless (which is why the Reach end never scans and Coldwater→Reach is the smuggling direction). Optional `pound` names the zone a SEIZED rig is parked in; without one an impound writes the weighbridge tile itself, which parks a forty-tonne truck on the plates the next inspection needs. Shape: { "name": "the Glacis Weigh", "pound": "zone_gatepound_lot" }.' },
    impound_pound: { label: 'Impound Pound', shape: 'flag', scope: 'zone', group: 'Zone: Flight',
      help: 'The lot a truck seized OUT ON THE VOID HIGHWAY is taken to. An inspection plaza is synthesised per crossing and torn down with it, so it cannot hold a rig itself — writing one into depot_zone is an address that stops existing. The plaza looks up the single zone carrying this flag instead, and with none authored anywhere it falls back to leaving the rig homed where it already was. Put it on the POUND INTERIOR, the room a driver walks into to pay; the release fee rides on the truck row, not here.' },
    vehicle_bay: { label: 'Vehicle Bay (drive-through)', shape: 'flag', scope: 'zone', group: 'Zone: Flight',
      help: 'THE ONE HOLE IN A BUILDING. Put this on a FACADE tile and a truck can drive through it — the tile still draws its building and is still solid to the flight sim, but the ground vehicle\'s obstruction probe returns clear (groundObstructionAt, windshield.js). It\'s what makes a depot a shed you drive OUT of rather than a wall the game teleports you past: `drive` now mounts the rig on the door tile, inside, facing the way its `entrance` points, and getting onto the road is something you do with the throttle. It applies to the WHOLE TILE rather than a doorway rectangle, deliberately — a door you can miss by four inches is a door players will hate — and the walls either side are ordinary solid buildings, so aiming badly still hits the shed. Never make this a general "buildings with doors are passable" rule: a pedestrian could always walk in, because walking in is what an exit is.' },
    truck_fuel: { label: 'Truck Fuel', shape: 'flag', scope: 'zone', group: 'Zone: Flight',
      help: 'A diesel pump a truck can `fuel` at. Redundant on a tile whose building_type is already fuel_yard — this is for a depot that keeps a pump without being one.' },
