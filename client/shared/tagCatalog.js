...
(function (global) {
  const TAG_CATALOG = {
    ...
    layer: { label: 'Layer', shape: 'enum', scope: 'class', group: 'Equipment',
      ...
      help: 'Which of the three worn layers this piece occupies on a body slot (underwear < outerwear < armor, innermost to outermost). Only one item per slot+layer; others see only your outermost layer. Also the order RAIN meets your clothes — an outer layer shields what is under it until it saturates. Ignored for weapon_hand and accessory. Defaults to outerwear if unset.' },
    ...
    emp_charge: { label: 'EMP Charge', shape: 'flag', scope: 'class', group: 'Gear',
      help: 'A single-use electromagnetic charge. `emp` fires the SAME pulse the ion storm does, scoped to your room — every unshielded `electronic` item carried by anyone present is `fried` until a bench repair, faraday containers spare their contents, and installed chrome blacks out. It does not care whose pockets it is in, including yours.' },
    explosive_charge: { label: 'Breaching Charge', shape: 'flag', scope: 'class', group: 'Gear',
      help: 'A single-use demolition charge. `breach <thing> with <charge>` opens the arming board, spends one, and leaves a fuse counting down on that object — after which anybody in the room, including you, is standing next to it. Deliberately NOT a weapon: there is no verb that throws one at a person, and the blast does not care whose it was.' },
    demolishable: { label: 'Demolishable', shape: 'flag', scope: 'furniture', group: 'Furniture',
      help: 'This object can have a charge wired to it. A builder decides what is worth blowing up when they place it, which is why `breach` reads a tag rather than a list of furniture ids. A detonation marks the row `destroyed` rather than deleting it, so the world keeps the scar.' },
    ...
    null_intrusion: { label: 'Null Intrusion Strength', shape: 'number', scope: 'class', group: 'Gear',
      help: 'On Null Gear: how many points of the target\'s security this platform cancels. Subtracted from the effective difficulty of every Null operation. 0 (or unset) is a bare-hands rig that helps not at all; 6 is a Black Box. Only the best Null device you are carrying is consulted.' },
    ...
    null_jam_radius: { label: 'Jam Radius', shape: 'number', scope: 'class', group: 'Gear',
      help: 'On Null Gear: how many tiles out the jam field reaches. 0 (or unset) means the room you are standing in and nothing else.' },
    ...
    no_install: { label: 'Cannot Be Installed', shape: 'flag', scope: 'class', group: 'Hardware',
      help: 'Wreckage. Marks chrome that death or a botched theatre destroyed — scrap value and repair stock, and nothing in the game will ever fit one to a body. Pair with Cannot Be Repaired.' },
    ...
    calibration_rig: { label: 'Calibration Rig', shape: 'flag', scope: 'class', group: 'Tools',
      help: 'Consumed by one CALIBRATE. Better rigs make the tuning board easier, never higher-scoring — a fine instrument makes the job easier, it does not make the machine better than it is. Not needed at a clinic bench.' },
    ...
    truck_bunkroom: { label: 'Truck Bunkroom', shape: 'flag', scope: 'zone', group: 'Zone: Flight',
      help: 'The room behind a depot office where a driver can stop the night (THE LONG HAUL). Four cots, a television, a fridge with something in it and a sink — every depot on the network has one and they are deliberately identical, because the point is that a driver who has been out for two days knows exactly what is waiting at the next one. Purely a LABEL: nothing gates on it, the cots are ordinary beds and the fridge is an ordinary container, so the room works whether or not anything ever reads this. It exists so that "where can somebody sleep on this network" has one answer to ask rather than five zone ids to remember.' },
    truck_yard: { label: 'Truck Yard (apron)', shape: 'text', scope: 'zone', group: 'Zone: Flight',
      help: 'The hardstand outside a depot door — the drivable tile a depot\'s `yard` points AT. Purely descriptive: it is what makes the street say there is a depot through that door, so a driver who parks on the apron can find the shop. The truth about which tile a depot uses lives in the depot\'s own `yard` key, never here, so a mismatch between the two is only ever a missing sentence and never a broken door. Value is the yard\'s spoken name.' },
