...
function build() {
  ...
  return [
    ...
    cat('THE VERBS', [
      `  pee ${dim('/ piss / urinate')}       ${dim('needs a toilet, or somewhere you are willing to use')}`,
      `  poop ${dim('/ shit / defecate')}     ${dim('takes a while, and you are not going anywhere during it')}`,
    ...
    cat('PRESSURE', [
      dim('  Eating and drinking fill two meters you cannot see. They never go down on their own —'),
      dim('  waiting does not help, and the only way down is a toilet.'),
    ...
    cat('FARTING', [
      ...
      `  ${dim(`Past about ${Math.round(floor * 100)}% full it stops being free: you are relying on a muscle that`)}`,
      `  ${dim(`is already struggling, and at its worst it fails roughly ${worst}% of the time.`)}`,
      dim('  If you get away with it, you will be told you nearly did not. That is your warning.'),
    ].join('\n')),

    cat('AFTERWARDS', [
      dim('  Stains go on your clothes, or on the floor if you were not wearing any. They stay'),
