...
export default async function regress({ run, check }) {
  ...
  // give-a-cigarette: the recipient-first grammar is claimed by the plugin (no
  // pack in hand → "don't have a cigarette to give"), an empty recipient prompts,
  // and a non-cigarette give falls straight through to the engine handler.
  r = await run('give bob a cigarette');
  check('give cigarette routed to plugin', /don't have a cigarette to give/.test(r?.message || ''), r?.message);

  r = await run('give a cigarette');
  check('give cigarette with no recipient prompts', /Give a cigarette to whom\?/.test(r?.message || ''), r?.message);

  r = await run('give bob sword');
  check('non-cigarette give falls through to engine', /Usage: give/.test(r?.message || ''), r?.message);
