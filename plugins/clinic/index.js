...
export function treatmentQuote(player, params = {}, npcId = null) {
  ...
  if (!missing && !bleeding) return { missing: 0, bleeding: false, cost: 0, free: false };
  ...
  if (onTheHouse) return { missing, bleeding, cost: 0, free: true };

  const base = Math.max(minimum, Math.ceil(missing * rate)) + bleedFee;
  const help = npcId ? relationHelp(player, npcId) : 0;
  return { missing, bleeding, cost: Math.max(1, Math.round(base * (1 - help))), free: false };
}
...
registerAction({
  ...
  handler: async ({ actor, params = {}, context }) => {
    ...
    const { missing, bleeding, cost, free } = treatmentQuote(actor, params, npcId);
    ...
    if (!missing && !bleeding) return { type: 'dialogue_line', text: pick(NOT_HURT) };
    ...
    return {
      type: 'dialogue_line',
      text: `Gloves, light, pressure.${bleedLine} Whatever was wrong with you is closed, cleaned and taped. You come off the table whole.${priceLine}`,
    };
  },
});
