...
import {
  ...
  SLOT_CAPS, BOTCHED_CALIBRATION_CAP,
} from './state.js';
...
import { cmdBackup, cmdAssurance, onRespawnZone, hasCortical, getBackup, CORTICAL, RESTORE_PRICE } from './backup.js';
...
// --- Seam 4: death ---------------------------------------------------------
// Skipped when the death was CLAIMED — jail took custody, or a cortical restore
// is about to write these exact rows back. See the ordering trap in death.js.
on('player.death', async ({ player, corpseId, claimed }) => {
...
async function listAugments(player) {
  ...
  if (mine.length) {
    msg += `\n<span style="opacity:.7">calibrate &lt;name&gt; · augment overclock &lt;name&gt; &lt;level&gt; · augment repair &lt;name&gt;</span>`;
  }
