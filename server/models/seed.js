...
async function seed() {
  ...
  // zone_city_west (Franchise Strip): force correct exits to Embassy (down)
  // and Clone Facility (in), and ensure east points to The Threshold.
  ...
  // zone_start (Clone Facility): ensure the 'out' exit returns to Franchise
  // Strip and building flags are set so it appears in the buildings list.
  ...
  const items = [
    ...
    ['item_scrap_armor','Scrap Vest',4.0,45,'common',{ description:'Metal sheeting over a leather jacket.', slot:'torso', armor:3, bulkiness:4, allowed_layer_range:{ min:4, max:5 } }],
    ...
    ['item_scrap_helmet','Scrap Helmet',1.0,20,'common',{ description:'A motorcycle helmet with extra rivets. Visor status: optimistic.', slot:'head', armor:2, bulkiness:3, allowed_layer_range:{ min:3, max:5 } }],
    ['item_cargo_pants','Reinforced Cargo Pants',1.5,18,'common',{ description:'Pockets for days. Knees patched twice over.', slot:'legs', armor:1, bulkiness:2, allowed_layer_range:{ min:2, max:4 } }],
    ['item_steel_boots','Steel-Toed Boots',1.2,16,'common',{ description:'Standard issue, several owners ago.', slot:'feet', armor:1, bulkiness:3, allowed_layer_range:{ min:2, max:4 } }],
    ...
    ['item_work_gloves','Work Gloves',0.3,8,'common',{ description:'Stained in ways you choose not to think about.', slot:'hands', bulkiness:1, allowed_layer_range:{ min:1, max:3 } }],
    ...
    [‘item_riot_vest’,’Riot Plate Vest’,5.5,140,’uncommon’,{ description:’Custodian crowd-control plating. Stops bullets and blades; useless against a live wire.’, slot:’torso’, armor_soak:{ kinetic:6, edged:4, energy:1 }, bulkiness:5, allowed_layer_range:{ min:4, max:5 } }],
    [‘item_insulated_gloves’,’Insulated Gauntlets’,0.6,95,’uncommon’,{ description:’Lineman’s gloves rated for things that arc. Padding is an afterthought.’, slot:’hands’, armor_soak:{ energy:5, fire:3, kinetic:1 }, bulkiness:3, allowed_layer_range:{ min:3, max:5 } }],
    [‘item_basic_shirt’,’Basic T-Shirt’,0.3,2,’common’,{ description:’Plain white cotton. Worn, faded, and slightly too large. Does almost nothing.’, slot:’torso’, armor:1, insulation:5, gets_wet:true, auto_equip:true, bulkiness:1, allowed_layer_range:{ min:1, max:2 } }],
    [‘item_basic_pants’,’Basic Pants’,0.5,2,’common’,{ description:’Gray work trousers, a little threadbare at the knees. They fit. That\’s about all they do.’, slot:’legs’, armor:1, insulation:8, gets_wet:true, auto_equip:true, bulkiness:1, allowed_layer_range:{ min:1, max:2 } }],
    [‘item_basic_shoes’,’Basic Shoes’,0.6,2,’common’,{ description:’Canvas sneakers, well broken-in. The left sole is starting to peel.’, slot:’feet’, armor:1, insulation:3, gets_wet:true, auto_equip:true, bulkiness:1, allowed_layer_range:{ min:1, max:3 } }],
  ];
