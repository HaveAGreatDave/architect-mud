...
export const SCHEMA_SQL = `
  ...
  --
  -- backer_id is deliberately NOT surfaced by default: a poster names nobody
  -- (see the anonymity section in docs/systems-bounties.md). unmasked_by is the
  -- list of player ids who have PAID to learn it, so the reveal survives a
  -- relog rather than being an in-memory courtesy.
  CREATE TABLE IF NOT EXISTS bounties (
    id             TEXT PRIMARY KEY,
    target_id      TEXT NOT NULL,
    target_handle  TEXT NOT NULL,
    backer_id      TEXT NOT NULL,
    backer_handle  TEXT NOT NULL,
    amount         INTEGER NOT NULL,          -- escrowed payout (fee already removed)
    fee            INTEGER NOT NULL DEFAULT 0,-- house cut, taken at posting, never refunded
    note           TEXT,                      -- the backer's line, printed on the sheet
    ...
    claimed_handle TEXT,
    claimed_at     BIGINT,
    unmasked_by    JSONB NOT NULL DEFAULT '[]'::jsonb
  );
  CREATE INDEX IF NOT EXISTS idx_bounties_open   ON bounties(target_id) WHERE status = 'open';
  CREATE INDEX IF NOT EXISTS idx_bounties_expiry ON bounties(expires_at) WHERE status = 'open';
  CREATE INDEX IF NOT EXISTS idx_bounties_backer ON bounties(backer_id)  WHERE status = 'open';
