...
//
// A gate is a PURE check: no side effects, no state writes. All gates run in
// registration order before any part of a move mutates state; the first
// { block: true } wins and its message is returned to the player. The engine
// registers its own laws (door locks, encumbrance) through this same chain in
// commands/movement.js, so laws and plugin gates are listable side by side.
//
// gate({ player, from, to, direction, door, opts }) → { block, message } | undefined
