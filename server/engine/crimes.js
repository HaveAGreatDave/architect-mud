...
export const CRIME_DEFAULTS = {
  ...
  breaking_and_entering: { label: 'Breaking and entering',        stars: 4,   witness: 'always', description: "Forcing a shopfront and letting the alarm call it in. The witness is 'always' because the ALARM is the witness — it dialled SPECTER-PD itself, so no camera roll applies and a dark street buys you nothing. Kill the panel inside the window the box gives you and none of this is charged; a shop with no alarm fitted falls back to the ordinary witness gate on the breach." },
  ...
  running_an_inspection: { label: 'Running an inspection plaza',     stars: 4,   witness: 'always', description: "Holding your lane past a lit inspection plaza on the void highway. Four stars because ignoring a lawful stop in a forty-tonne truck is not a paperwork offence, and because the counter-play is not stealth: the station says OPEN on the radio and on a gantry over your own lane, so anyone who drives past one has decided to. NOTE this row is a TARIFF rather than a witness rule - every room on that road is flags.lawless, so raiseCrime would swallow the charge on its first line even when forced, and plugins/trucking/plaza.js spends this star value through WANTED_RAISE instead. The witness column is what would apply anywhere the ordinary path could reach it." },
