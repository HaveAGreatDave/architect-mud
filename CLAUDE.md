...
- [docs/systems-flight.md](docs/systems-flight.md) — flight: aircraft, airfields, hazards, contracts, air combat, hangars/salvage/tuning, **rudder pedals + crash break-up death-cam + live sheared-surface flight** (as built)
- [docs/systems-helm.md](docs/systems-helm.md) — the **Echelon helm chase view**: a flight-renderer-reuse chase cam behind the yacht (wake, charted A\* courses, `sailT` + cast-off freeze fix), the heli-approach deck-cam cinematic, `echelon_bridge`-gated helm verbs (as built; standalone tablet mount pending)
...
- [docs/systems-ideologies.md](docs/systems-ideologies.md) — **ideologies** (the reworked factions): 4 canonical orders on a stance axis + categorical path, `ADJUST_REPUTATION`/`ADJUST_STANCE`/`ADJUST_PATH`, `player_ideology_rep`, the `ideologies`/`rep` command + Tablet reader, `org_relations` rivalry graph; what was renamed vs kept from `faction` (as built)
- [docs/systems-casino.md](docs/systems-casino.md) — **The Neon Vig casino**: self-contained `slots` plugin (`spin`, weighted reels, ~0.75 RTP) + a poker table reusing the `gametable` plugin (dealer, `summon` a gambler bot); the `game_tables` seed one-shot (as built)
- [docs/systems-terrain.md](docs/systems-terrain.md) — **terrain & the Terrain Painter**: `flags.terrain` ground-surface SSOT + server/client inference (`zoneTerrain`), the dev-panel Maps paint mode (brush/fill/pick/rect), Move-Building flow, pier→dock, and runtime consumption (minimap/tablet/pacing — not passability, not flight) (as built)
- [docs/reference/land-taxonomy.md](docs/reference/land-taxonomy.md) — **the land concepts and how they differ**: region (spatial `regions` table / `flags.region_id`, World Editor) vs district (land-use `districtFor`/`flags.district`) vs terrain (`flags.terrain`) vs biome (flight-render) vs `bp_district` provenance — one SSOT each, plus the confusions that make them easy to mix up. **Read before touching anything "district"/"region"/"terrain".**
...
- [docs/plugin-standard.md](docs/plugin-standard.md) — the `plugin.json` manifest schema (incl. the regress-enforced `objectGatedCommands`), the README convention, the tick/DB-burden rules, and the `regress.js` test-kit shape
...
- [docs/systems-swimming.md](docs/systems-swimming.md) — swimming: water tiles are swimmable (retired the boat-or-die gate), stroke/tread stamina cost scaled by a new Swimming skill (Endurance+Brawn), 0-stamina/out-of-breath drowning, diving to `flags.underwater` tiles on a breath timer; submersion soaks you (extends clothing-wetness) and pulls your core toward hypothermia via the body-temp `waterTemperature` path (as built; boat boarding + underwater content pending)
