...
| **synthesis** | Drug cooking — `cook`/`synthesize` reagents into potency-scaled drugs via a chemistry skill check + a "stabilize the reaction" minigame | `cook synthesize synthresolve` | Reuses the crafting recipe cache + 2d8−2d8 `skillCheck`. `cook <recipe>` validates reagents/skill/station (chem-lab furniture `flags.crafting_station='chem_lab'`, or a `cook_kit`-tagged item anywhere at −3) → sends `synth_minigame`; `synthresolve <id> <score>` (pending+TTL guard) folds the 0–100 minigame score + station bonus into the margin, consumes reagents, and produces a drug with `custom_data.potency` (read by `useDrug` to scale effects + OD weight). Botch = toxic damage. `chemistry` skill added to skills.js. Client minigame: [synthlab.js](../client/game/js/panels/synthlab.js). Seed: `server/models/temp/seed-synthesis.js` |
