...
| **Player decision ↔ the surface it is made on** | a command answers with text that is really an interface — a numbered list to answer, a shelf to buy from — so it scrolls away, nothing marks it as a decision, and there is no control to focus; plus dialogs outside the focus manager's shortlist that silently have no trap and no Escape | [log-vs-dialog-audit.md](log-vs-dialog-audit.md) |
