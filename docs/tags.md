...
`client/shared/tagCatalog.js` (served at `/shared/*`) exports `TAG_CATALOG`, a map of field name → `{ label, shape, scope, group, help, options?, refTable?, order?, targets? }`. `scope` is engine storage semantics: `class` (item template `tags`), `instance` (`player_inventory.custom_data`), `furniture` (`furniture.flags`), `zone` (`zones.flags`), `zone_column` (a real **column** of the zones table — see below). `shape` is one of `text|flag|number|enum|ref|list|object|range|hot|statmap` and drives the editor widget, serialization, and `shapeError()`. `order` sorts within a `group`. The optional `requires` names another key **on the same zone** without which this one is unreadable (`shop_price` needs `is_storefront`, `rent_cost` needs `is_apartment`): the Studio holds the field back until its parent is ticked and then reveals it in place, and `content:lint` **errors** on a child authored without its parent, since a value nothing can read is a bug rather than a preference. Declare these from what a key MEANS — a co-occurrence sweep over the zones will cheerfully claim `flightdeck` requires `cabin_window`, because zero counterexamples out of one authored tile is not evidence. The optional `targets` array (`['item','furniture']`) controls which dev-panel editors offer the tag and is set via the **Usable on** checkboxes on the Tags screen — a tag can be attachable on both (e.g. `broadcast_receiver`). When `targets` is absent, applicability is derived from `scope` (class→item, furniture→furniture, instance→neither). Both the item and furniture editors render the same dropdown-picker + chips UI, filtered by `tagAppliesTo(def,'item'|'furniture')`; the furniture picker stores values as flat keys in `flags`.
