...
| `portable_mediadeck` | broadcast | FURNITURE. The uplink for an on-location shoot: a stage that is not the channel's own studio needs one in the room or the signal never reaches the gallery. `deck_off` switches it off |
