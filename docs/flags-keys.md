...
| `berth` | flight (`state.js`) + GLASS (`windshield.js`) | open water a cargo ship works from, alongside a quay. `deriveSurfaceCell` turns it into `mark: 'berth'` and `drawFreighter` sails a coaster up the fairway, lies her against the wall while the gantries load her, and sends her off again — three hulls in turn, so the basin reads as a small fleet. ⚠ **The tile stays ORDINARY WATER, for `camp`'s reason pointing the other way** — a building tile joins the CFIT collision sweep and leaves the walk graph, so a berth authored as a building is a permanent ship-shaped hole in the basin you can fly into and cannot swim through, occupied or not. Put it on the water tile IMMEDIATELY OUTBOARD of the quay: the hull places itself off that tile's own edge on the quay side and the crane reaches to the same boundary, which is how two tiles in two passes agree without either being able to see the other. Shape `{ "fair": "north", "quay": "west" }` — the open water she comes from and leaves up (her bow points that way throughout), and the side she lies against. Every other number (beam, deck height, the fender gap, when she is alongside) is shared with the gantry and lives in the renderer, because one an author can get wrong here is a ship inside a quay wall. Authored on 1 tile, Basin Quay (911,902) |
...
| `curtain` | engine minimap + flight | **presentation only** — tile borders the Architect's energy wall on the city's land edge; renders as a line on the tile's own **outward edges** on all three 2-D map surfaces (derived once as `spec.curtain` — see `deriveCurtain` in `scripts/content/derive.mjs`; it was a shimmer ring around the whole tile until 2026-08-17, which read as *the tile is the wall* rather than *the wall is at this edge of it*), plus the windshield wall and a room-description curtain line. It gates nothing **on foot**: there is no move gate on this flag (grep `registerMoveGate`), and it is not what keeps a walker in the city. It IS solid to a **vehicle** (2026-09-15) — `curtainTopZAt` in `windshield.js` turns the drawn arms into collision geometry, `groundObstructionAt` returns it so a truck rebounds off it, and the aircraft CFIT sweep takes it as a `Math.max` against the building roof. Before that, both sims collided only against building mass, which a perimeter tile has none of, so a rig drove clean through a wall it could see the whole way. ⚠ **The `perimeter_gate` tile is the one that must not be solid** — `curtainRun` hands it the flanking wall's axis so the run butts into the pylons, so it carries `cur` like any other perimeter tile and is skipped on its `mark === 'gate'`, exactly as the renderer skips it. `scripts/shapes/curtain.mjs` holds both halves. The seal is 133 authored walls between the frontier tiles — `connections` rows with `blocked: true` and `name: "the Architect's Curtain"`, minted by `scripts/content/mint-curtain-walls.mjs` and held closed by `content:lint` + the regress curtain law. Until 2026-07-29 the seal was a derive-time rule reading `flags.district === 'wilds'`, which meant a district repaint could open a hole with no diff. There is no `perimeter` plugin — the engine whitelists these three into the map payload (`world.js:928`, `commands/movement.js:778`) and `plugins/flight/state.js:574` renders the wall |
...
| `camp` | flight (`state.js`) + GLASS (`windshield.js`) | people live on this tile in canvas rather than in a building. `deriveSurfaceCell` turns it into `mark: 'camp'` and `drawTentCamp` raises ridge tents, guy lines and a brazier out the canopy. ⚠ **The tile stays ORDINARY WALKABLE GROUND, and that is the whole reason this is a flag rather than a `building_type`** — a building tile leaves the walk graph and joins the CFIT collision sweep, so a tent city built out of buildings is one you cannot walk into and can fly an aircraft into. Authored on 10 tiles of the Pitch and Rag Row (docs/proposals/old-coldwater.md) |
...
| `entrance` | world (map arrows) | **authored** door side (`north\|south\|east\|west`) for the facade's map entrance arrow, read by `buildingEntranceDir` (`world.js:176`). Baked once from the road graph by `scripts/bake-building-entrances.mjs`, **not** inferred at runtime — inference let unrelated terrain painting silently relocate doors. The interior's `out` exit must mirror it. The yacht plugin is the one legitimate runtime writer (the Echelon's exterior tile sails). ⚠ **On a tile with no `facade` tag it is not a door at all — it is the model's HEADING.** `buildingEntranceDir` is facade-gated and answers null there, so `deriveSurfaceCell` falls through to the raw flag and hands it to `drawAuthoredModel`, which rotates the whole model by it. The three dock cranes (`zone_district_923_905`, `_923_906`, `_922_902`) carry `east` for exactly this reason: it is what puts the boom out over the water rather than inland. Removing it from a structure-only tile turns the model to face south and says nothing |
...
| `floor` | audio | **INTERIORS ONLY.** Authored floor surface (`boards\|carpet\|tile\|concrete\|stone\|metal\|dirt\|linoleum`) — what this room sounds like underfoot, for the footstep cue in the dense sound tier. The indoor counterpart to `terrain`, which is `null` indoors *by design* (an interior has no ground surface), so this is the only half of the question the world did not already answer. Unset falls back to `boards` rather than silence, so forgetting one costs a slightly wrong floor and never a missing sound. Seeded across 591 interiors by `scripts/content/seed-floors.mjs`, then hand-corrected |
| `scavenging_table_id` | scavenging | loot table for searching here |
| `ambient_pool` | ambience (engine `world.js`) | names the **sub-area voice** this tile speaks in — the `theme` of a set of `global_ambient_events` rows written for one stretch of country (`deadwater_the_wide_quiet`, `scarletwastes_hardpan`). Read by `getRandomAmbient` **ahead of** `ambient_theme`, falling through to it if the named pool is empty. A SECOND KEY on purpose: `ambient_theme` answers *what kind of place is this* and has other readers (ambient-life gates routines on it, `knock` tests it for `indoors`), so it cannot also carry 29 sub-area names. Added 2026-08-22, when the region builders' habit of stamping a sub-area's whole array onto every one of its tiles was found to be 9MB of the world load — 11,633 tiles carrying 29 distinct arrays to say 243 things. Leave it unset on anything hand-authored: a room with prose of its own puts it in `ambient_events`, which still wins over any pool |
...
| `bounty_buyer` | engine (vendor) | pays the specialist rate (70% vs 40%) for anything tagged `vermin_part` — proof-of-kill an exterminator sent you for. Same lever as `drug_buyer`/`food_buyer`, hung on the ITEM TAG rather than an id, so the next pest anybody writes is already worth something to the right buyer |
| `fights_for_you` | ally | consent gate for the `ally <name>` verb, exactly as `escortable` gates `escort`. Without it the verb refuses, so an NPC only ever fights for you because something in the world said it could |
...
| `posted` | engine (housing) | **this NPC never goes home — the post IS the life.** Excludes them from the commute build (`scripts/house-posted-npcs.mjs`): no apartment, no derived shift, no `GO_TO_WORK` graph. For fixtures and machines (the Compound Cashbot, Warden Unit "Threshold"), gate/compound personnel (the South Gate troopers, the Ascendant stronghold cast) and anyone whose workplace is their whole existence. `aa_crew`, `aa_engineer`, `police`, `haunt_zone` and `no_attack` already imply it, so those need nothing extra |
...
| `camera_droid` | broadcast | this NPC IS a working camera in whatever room it is standing in (unioned into `_camerasIn`). Dispatched by ordinary `npc_staff` scheduling; a downed, jammed or departed droid takes the picture with it. `camera_label` names it on air |
| `absent_lines` | broadcast | what this NPC says or does on the studio floor when a castmate has not turned up and the show cannot start. `{missing}` is the name(s); a line starting with a quote is SAID, anything else is DONE. Edge-triggered, one voice per absence |
| `preshow_act` | broadcast | `{stage, zone, before[], after[]}` — beats this NPC performs when a programme staged in `stage` enters its call window, and again once it is off air. ⚠ `zone` (where the act happens) is separate from `stage` (where the show is made): they are rarely the same room. Edge-triggered, RAM-only |
| `portable_mediadeck` | broadcast | FURNITURE. The uplink for an on-location shoot: a stage that is not the channel's own studio needs one in the room or the signal never reaches the gallery. Also a transmitter for the whole channel when bound to it by `uplink_channel`/`channel_id`. `deck_off` switches it off, written by the `uplink` verb |
| `delivery_lines` | broadcast | an NPC's OWN fumbles/off-script asides/collapses, keyed by delivery state with `any` as the catch-all. Authored **replaces** the house pool for that pool, never joins it |
| `tangents` | broadcast | an NPC's own off-script RUNS, keyed by delivery state (`{ loose: [[line, line, line]] }`; a run may be `{ lines, topic }` to declare what it is about). A run is picked entire, so it stays coherent. Authored **replaces** the house pool; `any` is consulted only for an unauthored state. See [systems-broadcast.md](systems-broadcast.md) |
| `wrangle_lines` | broadcast | the CO-HOST half of a tangent: `early` / `late` / `recover` registers of lines trying to get the show back, optionally nested under a run's `topic` (topic → register → `any` → house). Read off whoever else is standing on the studio floor, and spoken in their own voice at their own impairment |
...
| `dead_drop` | deaddrop | this container is findable **as a cache** by the `search.provider` in `plugins/deaddrop`. ⚠ It is a separate opt-in and must never be derived from `concealed`: 53 of the world's 58 concealed rows are planted spy devices, so a roll keyed on `concealed` would turn `search` into a counter-surveillance sweep and end SPECTER's plant-a-camera game. A cache carries all three of `container` + `concealed` + `dead_drop` |
...
| `vends` / `vend_line` / `vend_cooldown_s` | vending | dispenser machine: item id to dispense (required), flavour line, per-machine throttle in seconds (0 = off; defaults to 60 for a machine that MAKES what it hands over, 20 for one that drops a packet) |
| `vend_drink` | drinks | a recipe key from `DRINKS`. The dispenser fills the vessel it hands you instead of handing it over empty, and charges for it — this is what makes an espresso rig serve coffee rather than serve cups. Needs `brew_tier` beside it (the band and the price both come off the tier) and `vends` pointing at drinkware the recipe accepts; all three are swept by regress |
| `wakefulness` | drugs (engine) | **drug-row flag**, 0–1: how hard this drug holds your eyes open, as a share of what a real upper does. Read by `stimulantPotency`, which feeds the sleep refusal and the fatigue clock. Absent falls back to the old rule (`drug_class === 'stimulant'` is 1, everything else 0), so authoring it is the ONLY way a drug outside that class stays awake — deliberately, because `drug_class` is the additive overdose pool and `drug_family` cannot tell coffee from a cigarette. Coffee is 0.6 |
| `vend_price` | drinks | flat price override for a `vend_drink` machine, in credits. Unset derives it: the vessel's own `value` plus the tier's charge, because **you keep the cup**. Regress refuses one authored below what the vessel is worth |
...
| `card_mint` | cards | mint terminal — `mint` previews here for free and strikes for ₵2,500; `scrap` eats duplicates here too. **One in the world:** the press at Mint Condition (`zone_mintcond_floor`). The five civic presses that used to carry this flag (Compound Interest Hall, the deposit gallery, Permits Window 6, the records reading room, the Lucky Bastard house press) are still furniture and are now described as decommissioned — the key was *removed from the file* rather than the file deleted, because the import replaces the whole `flags` column and a deleted file leaves the prod row untouched |
| `tape_rental` | videostore | this furniture is a tape rental wall — gates `rentals`/`borrow`/`returntape`/`settle` |
| `tape_shelf` | videostore | the wall's stock, an array of item ids. **Authored, never inferred** — "every `beta_cassette` in the game" would put one shop's private stock on every wall the day somebody adds a tape elsewhere |
