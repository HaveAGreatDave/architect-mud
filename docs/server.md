...
| `vehicle.crewed` | `environment.js` `pulseEpicentre` (**sync**) · `plugins/weather/index.js` on `weather.empPulse` | `()` | **GATHERED** — `{ mapId, x, y, knockOut(untilMs) }` per crewed vehicle. Where an aircraft, a rig or the Echelon IS, and how to take its electronics out. Answered by flight, trucking and yacht. ⚠ **CREWED, not merely existing** — this list also decides where an EMP pulse can LAND, so an empty airframe on a ramp would be an epicentre rolled onto furniture. ⚠ **SYNC BY CONTRACT on the engine side** (`gatherHookSync` inside `pulseEpicentre`): contributors answer out of their own RAM registries with no `await` and no `query()`. See [systems-weather-extreme.md](systems-weather-extreme.md) |
...
| `shop.neverShuts` | `plugins/commerce/shopdoor.js` `shopClosedFor` | `(zone)` | **GATHERED** — truthy from any contributor means this room has no closing time, whoever trades in it. A shop is DERIVED (an NPC with stock and a timetable whose shift is here), which is right almost everywhere and cannot tell a shopfront from a room that merely has a trader working in it — Fairweather's covered dock indexed as the shipwright's shop and locked at six with the player's boat inside, while her shift is in there because `refit` wants a shipwright beside a covered berth. ⚠ Asked in `shopClosedFor` and nowhere else, because the move gate, the shut provider and the closing sweep all read it, and a carve-out in one of them is a door that says open and refuses. ⚠ **SYNC BY CONTRACT** — the shut provider is asked for every cardinal side of every interior tile in the minimap window ([powerboat](../plugins/powerboat/README.md)) |
| `swim.afloat` | `plugins/swimming/index.js` `syncSwimmer` | `(player)` | **GATHERED** — truthy from any contributor means this body is on top of the water rather than in it. Swimming decides who is treading from what they are **carrying** (a `boat`-tagged item, the flight mutation), which is all it can see on its own — so somebody sitting in a `boats` row was a swimmer to every rule it owns, and the tick bled their stamina until they drowned at the wheel of a working boat. ⚠ **SYNC BY CONTRACT** — asked on every move in the game and once a second for every body in the water, so a contributor answers out of its own RAM registry ([systems-swimming.md](systems-swimming.md)) |
