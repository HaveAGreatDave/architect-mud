...
**Ambient the body drifts toward.** `feltAmbientC(player, zoneId, tempOffset)` in [environment.js](../server/engine/environment.js) — **the one answer to "how cold is it where this body is"**, and the only thing that should ever be asked. It folds in the two things that are properties of the *player* rather than of the room (windproofing and a climate-controlled vehicle cabin, both below) on top of `getZoneApparentTemperature(zoneId, tempOffset)` — the "feels like" temperature (diurnal + per-tile weather offset outdoors, or a stored interior temp indoors; wind chill + humidity folded in outdoors only). See the apparent-temperature detail in [systems-world.md](systems-world.md); the temperature tick does not re-derive the curves. A **submerged** swimmer (`player._submerged`, owned by the swimming plugin) drifts toward `waterTemperature(zone)` instead and counts as fully wet regardless of gear — see [systems-swimming.md](systems-swimming.md).

### Vehicle cabins

**A room is a place; a cabin is a player.** Aircraft cockpits and truck cabs hold **20°C whenever the engine is running**, reach it within a minute or two of the key turning, and bleed it back to the weather once it stops. Mechanically this is `registerCabinProvider` in [environment.js](../server/engine/environment.js) — a plugin that owns vehicles answers *which cabins exist right now, who is sitting in them, and is the engine turning*; the engine owns the thermometer, the setpoint and both rates (the constants are in [systems-world.md](systems-world.md#vehicle-cabins)). Providers today: [plugins/trucking/hvac.js](../plugins/trucking/hvac.js) and [plugins/flight/hvac.js](../plugins/flight/hvac.js).

> **Why this is not a `registerHeatSource`.** A heat source warms a *zone*, and a driver's `current_zone` is the road tile the rig is over — a public outdoor tile with pedestrians on it. Warming the zone would heat the street. And a pilot's `current_zone` is the airfield they took off from (takeoff removes occupants from the zone's player set but has nowhere to move them to), so before this every thermal question about a pilot was answered by the weather on a patch of ramp they were four thousand feet above, with full wind chill besides. A cabin is climate control that travels with a **person**, which is exactly the distinction the seam draws: the driver and the pedestrian stand on the same tile at different temperatures.

The cabin **replaces** the ambient rather than offsetting it — it is climate-controlled air, so it takes no wind chill and no zone `temp_offset`. A cabin with the engine off still blocks the wind, because you are sitting inside it. A cabin nobody is in is dropped from the map entirely, so the next person to climb in starts from the weather rather than inheriting a journey they never took.

> **The room always wins.** Anything with real interior zones is already covered by the ordinary indoor HVAC above and is deliberately excluded from the cabin path: the **Echelon**'s thirteen rooms run off her engine-room junction box (`gen_zone_echelon_engine_jbox`, `fuel_burn_rate: 0` — she is always powered unless the engine is disabled), and the **Leviathan**'s walkable cabin rooms are `is_interior` zones wired into `power_zones` off the airframe. Both were already held at 20°C and needed no change. Treating them as vehicle cabins as well would have let a parked Leviathan with cold engines drag its own furnished lounge below the identical room in a building, for no reason a player could see. The exclusion lives in the flight provider — see the ⚠ in its header.

**Setpoint parity is deliberate.** A cab is not a *better* room than a room; the moment it is, every interior in the game becomes the wrong place to shelter. What a cabin buys is **speed**, not a higher number.

> ⚠ **The HUD thermometer does not know about cabins yet**, and the obvious fix does not work. An occupied cabin sits in an *outdoor* zone, and `broadcastZoneWeather` pushes that zone an `environment.zoneTempTick` every 30s — the client reads the presence of its `cloudCover`/`precipRate` keys as "outdoor tick" and their absence as "indoor room". A per-player cabin push on the same wire fights the zone push twice a minute: the thermometer flicks between 20°C and the weather, and the rain overlay toggles with it. The body-temperature model is per *player*; that wire is per *zone*. Doing it properly needs a client-side "I am in a cabin" latch. Until then the number on screen is the air outside the cab — the **simulation** is correct, the readout is not.
