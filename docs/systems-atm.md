...
`deposit` and `withdraw` use the engine's `transferCredits(player, amount, type)` for the basic credit movement — this enforces the "credits can't go negative" invariant in one place. The subsequent `cash_stock` update is a direct `query()` call after the transfer succeeds, so there is a brief window where credits have moved but `cash_stock` hasn't updated yet. This is acceptable for a game; there's no true ACID transaction wrapping both writes.
