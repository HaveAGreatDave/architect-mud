...
⚠ **A cell is sized in TILES, and its count scales with the area it is scattered over.** Until 2026-09-10 the radius was `span * (0.22 + rand() * 0.18)` — a fraction of the extent of the *whole* outdoor map. That reads as scale-independence and is the opposite of it: every cell in the world grew whenever a region was added anywhere. When Coldwater was the map, `span` was 92 and a cell came out 20-37 tiles across a 93×52 region — a front with an edge that crossed you. Deadwater, Terminus and the Scarletwastes (2026-08-11/12) sit far apart on a shared bounding box, which stretched `span` to 513, so a cell became **113-205 tiles**: two to four times wider than all of Coldwater, and always taller than the map. Nothing broke and nothing was removed — every tile simply sat deep in the smooth middle of one blob with no edge to reach it. Measured over Coldwater on a rain day, 92% of tiles read the identical map-wide floor, the region's whole cloud spread was 0.018, and a tile watched for two real hours moved 0.07: a solid overcast sheet, drawn by a system that still believed it was drawing weather. `CELL_R_MIN`/`CELL_R_RANGE` restore the 20-37 tile front. The counts beside them were tuned on that same 93×52 map, so a fixed radius alone trades one flat sky for an empty one — they are multiplied by `density`, the spawn area over `CELL_REF_AREA`, which is ~17 today. ⚠ **That makes the cell list ~60 long, not single-digit** — free for the sampler (150k samples in 4 ms) and *not* free on the wire, so the flight/cab packet crops to `CLIENT_CELL_RANGE` around the viewer (see `weatherFieldForClient`).

⚠ **The cells are not the whole sky, and two floors sit under them.** `sampleWeatherAt` *opens* at `field.baseCloud` — the day's ambient cloud, 0.4 cloudy / 0.7 overcast / 0.72 precip / 0.8 storm — and only then takes the max over the cells; and `precipFloor` (engine) floors every outdoor tile's rate at the day's headline `state.precipRate`, which is what makes light rain both seen and heard everywhere rather than only under a passing cell. Any consumer that reads the cells alone is describing a different sky from the one the ground is standing under: the flight sim did exactly that (no `baseCloud` on the snapshot, no precip floor on the wire) and about half the map flew clear over players in a downpour, and the dev panel's own weather map painted three cells on a blank grid for the same reason. Both fixed 2026-08-31 — `baseCloud` is on the snapshot, `precipFloor`/`floorType` ride the flight field packet, and `getWeatherMap` calls `precipFloor`. **The floor must also rise with the weather:** it did not until the same date (storm 0.5, rain 0.45, both *below* overcast's 0.7), so forcing Max Storm made most of the map less cloudy than a grey Tuesday. `plugins/weather/regress.js` now asserts the ordering.
