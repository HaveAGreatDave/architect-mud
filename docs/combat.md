...
Both directions use the same shape (`combat.js`):

```
roll = 2d10 (range 2–20)
total = roll + attackSkill
margin = total − targetDodge
hit = margin >= 0
```

- **Player → enemy:** `attackSkill = effectiveSkill(player, weaponSkill)`; `targetDodge = dodge_base (5) + enemy.defense`.
- **Enemy → player:** `attackSkill = enemy.stat_agi / 2` (lean enemies have no skill rows); `targetDodge = dodge_base (5) + effectiveSkill(player, 'dodge')`.
...
damage = max(1, damage − soak(struck part, damage_type))
```

Tunable defaults (`tunables.js` / `combat.js`): `crit_threshold = 8`, `crit_multiplier = 1.5`,
`head_damage_multiplier = 1.5`, `dodge_base = 5`, `soak_mismatch_factor = 0.25`.

### Body parts

`rollBodyPart()` does a weighted pick from `body_part_weights`
(default `head:10, torso:40, left_arm:12, right_arm:12, left_leg:13, right_leg:13`). The struck
part maps to an armour slot via `PART_TO_SLOT`: head→head, torso→torso, arms→hands, legs→legs.
**Feet have no body part in the weight table, so the `feet` armour slot never soaks anything in combat.**
...
(no code applies a weapon-triggered status effect — see the effects note below).
...
3. **Attack pacing:** `enemyAttackPlayer` only swings if `now − lastAttack >= 5000 − stat_agi×150` ms.
...
- credits `credit_reward` to the player,
...
`xp_reward` is plumbed from the enemy template through to the result but **never awarded** — there is
no XP stat; advancement is IP, minted from skill use. Treat `xp_reward` as vestigial.
