...
2. ✅ **The three engine seams** — **built, green (2026-07-18).**
   - **Soak** — `recomputeArmor` ([inventory.js](../../server/engine/commands/inventory.js)) gained a synchronous `registerArmorContributor` chain (protection.js provider model) + a shared `addSoakToSlot`. The augments plugin registers a contributor that reads installed augments, layers each augment's per-slot `soak` jsonb into `player.soak`, and sets `player.chromed`. Runs on login/equip, never a per-swing hot path.
   - **Mutation block + burn** — `checkMutationTrigger` bails on `player.chromed`; new engine `burnAllMutations()` (reverses stat_modifiers, clears `visibly_mutated`) is called by the plugin on the FIRST install (the flesh→machine conversion). `player.chromed` is a memory flag maintained by the plugin (armor contributor + a `player.login` handler) — derived, never a stored column. (The Threshold gate at [ascendant/index.js](../../plugins/ascendant/index.js) already read `chromed`; now it's live.)
   - **Backup respawn** — `player.respawnZone` hook in the augments plugin lands a non-jailed death at the Vats and rolls inventory+credits back to the last snapshot (regenerated row ids remap container refs; since-save gear destroyed). **Jail wins:** the hook mirrors jail's claim test (wanted peak ≥ 1 ⇒ yield). The respawn-override contract gained `skipOutfit` so the restored wardrobe isn't overwritten by starter kit.
...
6. ✅ **The Threshold move-gate** — `plugins/ascendant/` registers `ascendant:threshold`: inner-ring tiles (`ascendant_campus`, minus the public `ascension_gate` face) refuse the uncleared from either side; rushing (`player.running`) draws a turret warning, and **forcing it again inside a 12 s window makes the turrets actually fire** — soak-reduced energy bursts via `applyStrikeToPlayer`, escalating and lethal (routes to `handlePlayerDeath` with cause "Cut down forcing the Threshold"). Cleared = `ascendant_clearance` flag / Ascendant rep ≥ Known / already chromed. **Built, green (9 checks).**
...
8. ✅ **Halcyon prepaid-policy vendor + backup save-point** — **built, green (2026-07-18).** The Halcyon Underwriting desk carries a new `assurance_policy` flag (the secret front); `assurance [buy n]` sells prepaid restores (₵2500 each, eligibility-gated on owning the Cortical Backup augment — the reveal breadcrumb, since Halcyon billing *is* the Ascendant resurrection desk). `backup` at the Vats Registry (`ascendant_registry`) snapshots inventory+credits. Verb named `assurance`, not `policy` — the insurance plugin already owns `policy` (aircraft hull). New `player_backups` table (runtime-classed).

**Missing catalog rows authored (2026-07-18):** the four soak/special augments the step-1 build left out — Subdermal Weave (torso soak), Neural Co-processor, Rad-sealed Chassis (torso energy/rad soak), Cortical Backup (Inner Circle, enables the loop). Catalog is now the full 8.

**Live locally (2026-07-18):** campus + all 8 augments + the Halcyon flag imported (`content:import`, marker at `b0ed723b`). `augment install/remove` work at the Clinic (`augment_clinic`), `backup` at the Registry, `assurance` at Halcyon Underwriting.
