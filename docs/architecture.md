...
**Which city plant a junction box answers to** is resolved by `powerAnchorOf` + the plant choice in `installGenerator`: **region first**, then nearest, then a tie broken by generator id so the answer is the same on every machine and every re-run.

⚠ **It cannot be asked of the box's own coordinates, and asking anyway was a live bug until 2026-09-22.** A junction box lives in a utility ROOM; every interior in the game sits at grid 0,0 on its own `map_int_*` map, and the old form tested `grid_x != null`, which 0 passes — so it measured every box from the origin. Two of the four city plants are themselves interiors at 0,0, both scored distance 0, and a strict `d < minDist` handed the tie to whichever row Postgres returned first (the SELECT had no `ORDER BY`). **52 buildings ended up on another region's plant** — 49 Coldwater ones drawing from Terminus's substation, 3 in The Reach drawing from Coldwater. Power still flowed, because distribution does not care about distance, which is exactly why it went unseen. Repaired by `scripts/content/repoint-junction-boxes.mjs`.

⚠ **Rejecting 0,0 is NOT the fix, and that near-miss is the part worth remembering.** **251 zones carry non-zero coordinates on a map that is not the world** — apartments laid out at (1,0) and (-1,0) on `map_interior_*`, the Leviathan's flight deck at (0,-1) on `map_aircraft_leviathan` — and those are *local layout* positions that mean nothing to a distance. A rule that only excluded 0,0 measured the Leviathan from one tile off the origin: the same bug with a different number in it. **The test is `map_id === 'map_world'`**, the single real map (every region lives on it; the Under is its z −1 layer). Everything else is a local frame.

An interior reaches the world map through `flags.world_exit_zone`, which on an interior names the **facade** — followed exactly one hop, and only from a zone not on the world map itself (on a facade the same key means the *street*, so a second hop would walk off the building). A utility room is the room in a building *least* likely to have one, being a cupboard reached from inside, so the search falls back to the building network: that takes the unresolvable cases from 15 to 2 (an airfield hangar and the Leviathan's cabin, neither of which has a street door to name). An unresolvable box keeps whatever it has rather than being guessed at. `flags.offgrid` — the Echelon's engine room — is a deliberate `city_generator_id: null`: a ship makes her own power and is on nobody's grid.

Pinned by nine checks in the regress suite, which test the **rule** rather than the outcome, because the outcome is content and moves.

**Lighting.** Three kinds of light fixture exist as `furniture` rows (`is_light`, `light_on`, `light_type`):
...
- **Off-grid** (`gas`, `oil`, `candle`, `carbide`, `flame` — `OFFGRID_LIGHT_TYPES`) — a fixture that makes its own light. An ordinary `object_type: 'light'` row, so `look`, `examine`, the lumen tally and the dev power map read it with no change of their own, and it differs in exactly two ways: **the grid cannot switch it off, and it never draws from the grid.** A lit one raises its zone's `has_emergency_lighting`, which is already exactly "this room has light when the grid does not" and lands the room at `EMERGENCY_LIGHT_LEVEL` — dim, and not dark.

  ⚠ **Both halves are silent when they are missing, which is why this is a type and not an authoring convention.** The offline branch cuts every `object_type='light'` in a dead zone, so a lantern authored the obvious way is switched off by the grid it does not use; and `drawKwFor` falls through to `DRAW_DEFAULT_W` for an unrecognised `light_type`, so a room lit by paraffin bills the city for it and can help brown out its own junction box. Nothing in either case logs anything. The brownout branch excludes them too, for a third reason of its own: it sheds lights cheapest-first against the remaining pool, and a fixture drawing 0 would sit at the head of an auction it is not bidding in.

  This is what the off-grid regions have always *described* and never modelled — Deadwater and the Under are dark by construction and every light in their renderer arms is flame, oil or carbide. First users are Old Coldwater's eleven rooms ([proposals/old-coldwater.md](proposals/old-coldwater.md)).
...
**Three sync, query-free readers**, all O(1) Map reads off live RAM and all called per cell of a ~73×73 flight window by `deriveSurfaceCell`, so none of them may ever grow a query: `getZonePowerStatus` (powered / overloaded / offline), `getZoneEmergencyLighting` (does this zone keep a light when the grid drops — the sim has costed it indoors as `EMERGENCY_LIGHT_LEVEL` since it shipped), and `getZoneOnGrid`. ⚠ **The last one exists for the renderer and not for the sim.** A `power_zones` row with no `generator_id` is a zone that was never connected — 4,836 Deadwater tiles and 117 in the Under — and to everything inside `environment.js` that is the same as a fault: no light, no HVAC, no ATM. Out of a cockpit they are opposites, because those buildings are lit by flame, oil and carbide and have never had anything to do with a grid. A dead generator is still a generator; what this answers *no* to is a zone with no connection to lose. See [world-rendering.md](reference/world-rendering.md#buildings-in-a-blackout).
