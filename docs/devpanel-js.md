...
- **Plan map base** (`PLAN_MAP_INK`, `PLAN_TERRAIN_TONE`, `PLAN_TILE_DEFAULT`, `PLAN_TILE_BUILDING`, `planTileTone()`, `planTileIsBuilding()`, `planRegionOf()`, `planRegionName()`, `planTileZoneFor()`, `planRegionBuckets()`) — the monochrome one-region-one-floor silhouette a coloured overlay is drawn on. Two panels use it: the Enemies panel's spawn heat map and the Power panel's Region Map. It lives here so the terrain palette can't drift into two pictures of the same city, and `planTileZoneFor()` is the one implementation of "which tile does this interior room paint onto" (it walks up through nested interior maps to the facade). ⚠ **What makes a div-per-coordinate grid affordable is that it's bounded to one region on one floor and each cell is a bare few-pixel div** — no text, no per-cell exits. That same shape over every placed zone is what froze the old City Grid view at ~888 zones. The `.plan-tile` / `.plan-tile-sel` classes in `styles.css` are its only styling.
...
- **District painter** (`toggleDistrictMode()`, `setDistrictKey()`, `setDistrictTool()`, `districtPaintStart/Over/End()`, `districtPick()`, `districtFill()`, `districtRectStart/Over/Commit()`, `districtPanelHtml()`) — paints `flags.district`, the land-use identity a tile carries. Same four tools as the terrain painter (brush / fill / rect / pick) on purpose, so it needs no separate learning. **Its palette needs no content file of its own**: `content/districts/*.json` already carries `id` + `name` + `color`, served by `GET /districts` and cached by `ensureDistrictData()` in `panels/zones.js` (which loads first — load order is a contract). Two deliberate differences from the terrain painter: it renders the **resolved** district and marks its source (`_distResolve` — solid = authored, striped = inherited from the zone id prefix, faint = falling through to the `residential` default), because an unpainted city silently reads as one enormous Residential Blocks *in the game too*; and it **never conjures a tile**, since a district is an identity carried by ground rather than a kind of ground. Buildings are painted as well — a shop in the Redline is in the Redline. The panel's per-floor authored/prefix/fallback counts are the number a district-assignment pass works against.
...
- **View controls**: `setPowerPanelMode()`, `setPowerPanelView()`, `setPowerPanelBuilding()`, `setPowerPanelInteriorZ()`, `_buildInteriorMapHtml()`. Three views: **Region Map** (the default the panel opens on), **City Grid** (the plant → junction-box schematic) and **Building Interior** (one building's rooms). There is no Generators table any more — a flat list of every generator under a map that already shows all of them was the same information twice, so `_powerGenActionsHtml()` / `_powerGenRowHtml()` put toggle, capacity, zones and remove on the plant card beside the thing they act on, and `_powerOffPlantHtml()` keeps the two cases that have nowhere on the map to live: off-grid boxes (a building generating its own power on purpose) and boxes wired to no plant (a fault).
- **Region Map** (`_buildRegionMapHtml()`, `_powerGridHealthHtml()`, `_powerRegionGridHtml()`, `_powerRegionLegendHtml()`, `_powerRegionDetailHtml()`, `_powerByTile()`, `_powerPlantsByTile()`, `_powerPlantFor()`; controls `setPowerRegion()`, `setPowerRegionZ()`, `powerRegionStepZ()`, `powerRegionSelect()`, `powerRegionSelectPlant()`) — the grid laid back over the ground, on the shared plan base from `maps.js`. City Grid answers "what is wired to what" and throws the geography away; this answers "where is the grid, and is it well". Click a building for its draw, its supply, its status and which plant feeds it; click a plant for its capacity against its demand and everything on it. **Every power-model zone is folded onto the tile it paints**, so a building carries the sum of its rooms and — deliberately — **the status of its worst one**, because one dead floor is the thing you opened this map to find and an average would hide it. Colour carries status, alpha carries draw. ⚠ **Draw is not a status**: `current_load_kw` is what a room's lights and appliances pull *this second*, and on a healthy grid almost nothing is pulling anything — 17,108 of 17,239 zones read `powered` while 258 are drawing. The first cut recoloured a zero-draw zone as "idle", which repainted 99% of a working city grey and left three green tiles on the map. A powered tile pulling nothing is still powered, so the alpha ramp has a floor rather than a separate colour; offline keeps a flat alpha because it's a state, not a magnitude. The engine's statuses are `powered` / `overloaded` / `offline` — `unpowered` is only the fallback for a zone that isn't on the power model, and such a tile gets no overlay at all. A plant's tile comes from `map_zone_id`, which the server already resolves out of an interior to the exterior zone. The health header sums the **whole region**, every floor, since a plant is regional and a basement is still on it.
...
- Holds `powerPanelGenerators`, `powerPanelMode`, `powerPanelView`, `powerPanelBuilding`, `powerJbByOutdoor`, and the Region Map's `powerRegionId`, `powerRegionZ`, `powerRegionSel`, `powerRegionPlantSel`, `powerPanelMapParents`, `powerPanelRegions`.
