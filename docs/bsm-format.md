...
| `@acted` | `meta.acted` → `media_broadcasts.acted`. Declares the programme is **performed by NPCs standing in a room** rather than printed. Staffs the cast (they commute to the stage) and presence-gates the airing, so an empty floor falls to camera-idle then technical difficulties — exactly what a `live` channel does, but as a property of the SHOW, so it works on a `playlist` channel without dragging that channel's films and ball games onto the same gate. Takes no value; the directive's presence is the opt-in. ⚠ Omit for a recording — a drama names its actors with `npc_anchor` too, and gating one on its cast being awake takes it off the air. |
