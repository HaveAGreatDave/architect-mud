...
7. **Ship it through CODEX.** Injecting via the API against your **local** server writes the DB row **and** (via the dev-panel save-hook) its `content/<table>/<pk>.json` file — so the file is already sitting unstaged in your working tree; it's just not committed yet, which means it's still invisible to the team and lost on the next rebuild until you commit it. Content lives in git as one JSON file per entity; the DB is a build artifact. **Invoke the [`codex` skill](../codex/SKILL.md)** to run the exit gate: **do NOT `content:export`** (the file is already written) → `git status` to review what the save-hook wrote → `content:lint` → `npm run test:regress` → commit. A design task is not done until CODEX ships it. See "What not to serialize" below for the one thing you must get right, and [docs/content-pipeline.md](../../../docs/content-pipeline.md) for the full model.
...
- [ ] **Greeting split — ask about this every time you author a `root` node.** Three different lines, three different jobs: `text_by_relation.first` is the introduction they only ever get to make ONCE (name, nickname, what they're standing there for); the node's ordinary `text` is what they say EVERY DAY to someone who isn't a regular yet; `known`/`familiar`/`close` are what they say once you are. `stranger` is *not* first-meeting — it repeats for several visits, so an intro line left in plain `text` gets recited forever. Nothing is required and everything falls back, so ask the user which of the three they want written rather than assuming. See [systems-relationships.md](../../../docs/systems-relationships.md#the-introduction-they-only-get-to-make-once--first); `content/npcs/npc_grady.json` is the worked example.
