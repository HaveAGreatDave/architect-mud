...
- **Item PUT is a full-object replace with NO merge/COALESCE** (`apiUpdateItem`): it writes `name,type,weight,value,rarity,tags` straight from the body, so a *partial* PUT (e.g. just `{tags}`) NULLs name/weight/value/rarity. Always PUT the complete item object — keep the create payload in scratchpad and edit it, never hand-write a diff. Note it also ignores `subtype/effects/flags/is_stackable` on update (those are create-only columns).
