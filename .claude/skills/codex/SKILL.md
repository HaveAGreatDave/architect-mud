...
- **Content deploys are debounced — a push does NOT go live immediately.** Each deploy reboots Render, which cold-reloads the whole world from Neon (~36MB egress); deploying one-for-one on every content commit blew the free plan's transfer cap (July 2026). So a content push now only *validates* (regress); the actual deploy runs on a **2-hourly schedule** that batches the commits since the last run. See [.github/workflows/deploy-content.yml](../../../.github/workflows/deploy-content.yml).
- **To force an immediate deploy / server refresh, put `[deploy]` in the commit message.** When the user wants the change live *now* (not on the next schedule tick) — "push it live", "refresh the server", "deploy this now", "I need to see it on prod" — add the literal token `[deploy]` anywhere in the commit message. That commit deploys as soon as CI passes instead of waiting for the schedule. (The other force path is the manual Actions-tab "Run workflow" button, which the user drives themselves.) A normal commit message = batched; `[deploy]` = now. When you commit at the user's request, ask whether they want it live immediately if it isn't obvious, and add the token when they do.
