...
- Production gets the schema through the **CODEX pipeline**: `content:import` (locally, and in CI on push to `main`) applies `SCHEMA_SQL` before importing rows, so DDL reaches prod inside the normal content deploy. Don't run `db:schema` against prod. Only *data transformations* on existing rows (backfills/rewrites) need a hand-written prod one-shot (`node --env-file=.env.prod scripts/<name>.mjs`).
- **Classify the new table in the content registry.** Every `CREATE TABLE` in `SCHEMA_SQL` must be classified in [`server/models/content-registry.js`](../../../server/models/content-registry.js) — `npm run test:regress` (layer 1a) fails until it is. **A persisted substrate is almost always `runtime`** (regenerated/accumulated at play time, never authored, never shipped) — the common case here. Only if the substrate holds *authored* rows someone edits is it `content` (then mind the `excludeColumns` self-healing-only rule — see the [`codex` skill](../codex/SKILL.md)). If it IS content, ship the initial rows through `codex`.
...
5. **Kill any background server you started.** The harness shares the Neon pool (pool_size 15); orphaned `node server/index.js` processes exhaust it (`EMAXCONNSESSION`) and time out real users. Player stat columns are `stat_brawn`/`stat_reflexes`/… (not `brawn`).
