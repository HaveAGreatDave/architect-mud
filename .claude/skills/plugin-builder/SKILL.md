...
- **Schema:** add the DDL to `SCHEMA_SQL` in `server/models/schema.js` (idempotent), apply it to your **local** dev DB with `npm run db:schema`, and **let the Relay deploy carry it to prod** — the _Deploy content → Production_ lane (`npm run relay`) applies the full `SCHEMA_SQL` (embedded in the dump) + additive content to prod in one regress-gated, backed-up transaction. So `CREATE TABLE IF NOT EXISTS` / `ADD COLUMN IF NOT EXISTS` / deferrable-constraint swaps all reach prod through a normal deploy — **you do not run `db:schema` against prod, and you rarely need a hand-written prod one-shot.** The one thing the additive deploy can't do is *transform existing rows* (backfills, moving data between columns) — write a one-shot for those and run it against prod after the deploy. **Never add a boot-time migration.**
- **Export allowlist — classify EVERY new table (the silent-seam trap):** the dev-panel backup, the git seed (`export-seed.mjs`), and the prod deploy all run the **same** `buildDump()` over the **one** `CONTENT_TABLES` list in `server/api/backup.routes.js` (there is no longer a second list — a duplicate one silently drifted and dropped flight's `aircraft_types`/`aa_sites` from the git seed while the dev-panel path stayed current). A new content table's rows are dumped only if you add it to `CONTENT_TABLES` **in FK-safe order**; if the table is runtime/player data that must *not* be seeded (per-player state, auto-created-per-furniture, generated league/board data), add it to `EXCLUDED_TABLES` instead. Declaring it in `plugin.json` `dataSchema` is *not* enough — that's documentation, not wired to the export. **Every `CREATE TABLE` in `SCHEMA_SQL` must be in exactly one of the two lists; `npm run test:regress` fails until you classify it** (layer 1a, the anti-drift guard). Skip classification and either your content restores **empty** on a fresh DB with no error (the quests/flight bug) or player data leaks into the shared seed. Watch FK ordering: the whole dump is one transaction, so a table inserted before its parent aborts the *entire* restore. For a genuine cycle **or a self-referential FK** (a content table referencing itself — e.g. `zones.parent_zone → zones`, `generators.city_generator_id → generators`), no insert order can satisfy an *immediate* check, so make the FK `DEFERRABLE INITIALLY DEFERRED` and rely on the dump's `SET CONSTRAINTS ALL DEFERRED` (see the `media_*`, `zones`, `generators` swaps in `schema.js`). Don't reach for `SET session_replication_role = replica` — Supabase restricts that role, so the prod deploy can't use it. A failed restore leaves an EMPTY DB whose only symptom is boot dying on the *first* query (`relation "server_settings" does not exist`) — validate a fresh restore actually COMMITs. Filtered subsets use `{ table, where }` (e.g. only `is_npc=1` orgs, only `owner_type='player'` apartments) so runtime rows in a mixed table stay out.
