...
- **Export allowlist (the silent-seam trap):** a new content table's rows are dumped (into the git seed *and* the prod deploy) only if you add the table to `CONTENT_TABLES` in `server/api/backup.routes.js` (in FK-safe order). Declaring it in `plugin.json` `dataSchema` is *not* enough — that list is documentation, not wired to the export. Skip this and your content restores **empty** on a fresh DB with no error (this is the quests-table bug). Watch FK ordering: the whole dump is one transaction, so a table inserted before its parent aborts the *entire* restore — for a genuine cycle, make the FK `DEFERRABLE` and rely on the dump's `SET CONSTRAINTS ALL DEFERRED` (see the media_* tables).
