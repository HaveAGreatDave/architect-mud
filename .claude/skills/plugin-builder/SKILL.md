...
4. **Kill any background server you started** before finishing. The harness shares the Neon pool (pool_size 15); orphaned `node server/index.js` processes exhaust it (`EMAXCONNSESSION`) and time out real users. If the harness dies with that error, hunt orphans or wait ~90s.
