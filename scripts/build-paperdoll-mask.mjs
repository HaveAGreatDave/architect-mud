// One-shot asset transform: read the white-on-black paperdoll and emit a
// transparent-background silhouette (RGB=white, alpha=luminance) so it can be used
// as a plain CSS alpha mask tinted to the accent colour. Not a build step — run once.
import fs from 'fs';
import zlib from 'zlib';

const SRC = 'client/game/assets/paperdoll.png';
const OUT = 'client/game/assets/paperdoll-mask.png';
...
// Build the output: filter byte 0 per row, RGB forced white, A = luminance of source.
...
for (let y = 0; y < H; y++) {
  ...
  for (let x = 0; x < W; x++) {
    ...
    const r = cur[i], g = cur[i + 1], bl = cur[i + 2];
    const luma = Math.round(0.299 * r + 0.587 * g + 0.114 * bl);
    out[q++] = 255; out[q++] = 255; out[q++] = 255; out[q++] = luma;
  }
}
