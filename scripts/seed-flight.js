...
const TYPES = [
  { id: 'ac_mayfly',    name: 'Mayfly',    class: 'ultralight', takeoff_mode: 'stol', seats: 1, cargo_capacity: 0,   max_takeoff_weight: 90,   fuel_capacity: 20,  fuel_burn_base: 1.0, fuel_type: 'avgas',   altitude_ceiling: 1, cruise_speed: 1, handling: 2,  hull_hp: 8,  hardpoints: 0, noise: 1, price_buy: 1200,  price_rent_hourly: 40,   data: { feel: 'Cheap trainer you\'ll wreck; the wind tosses it.' } },
  { id: 'ac_dragonfly', name: 'Dragonfly', class: 'heli',       takeoff_mode: 'vtol', seats: 2, cargo_capacity: 40,  max_takeoff_weight: 320,  fuel_capacity: 40,  fuel_burn_base: 1.8, fuel_type: 'avgas',   altitude_ceiling: 2, cruise_speed: 2, handling: -1, hull_hp: 18, hardpoints: 0, noise: 2, price_buy: 8000,  price_rent_hourly: 120,  data: { feel: 'Versatile early pick; thirsty; hovers, lands anywhere flat.' } },
  { id: 'ac_mule',      name: 'Mule',      class: 'prop',       takeoff_mode: 'stol', seats: 3, cargo_capacity: 180, max_takeoff_weight: 620,  fuel_capacity: 70,  fuel_burn_base: 1.3, fuel_type: 'avgas',   altitude_ceiling: 2, cruise_speed: 2, handling: 0,  hull_hp: 30, hardpoints: 0, noise: 2, price_buy: 14000, price_rent_hourly: 220,  data: { feel: 'Freight workhorse; rugged; rough-strip capable.' } },
  { id: 'ac_leviathan', name: 'Leviathan', class: 'heavy',      takeoff_mode: 'strip', seats: 6, cargo_capacity: 600, max_takeoff_weight: 1400, fuel_capacity: 160, fuel_burn_base: 2.6, fuel_type: 'jet',     altitude_ceiling: 2, cruise_speed: 1, handling: 3,  hull_hp: 55, hardpoints: 0, noise: 3, price_buy: 60000, price_rent_hourly: 900,  data: { feel: 'Economy endgame; slow, hungry, needs real runways.' } },
  { id: 'ac_reaper',    name: 'Reaper',    class: 'gunship',    takeoff_mode: 'stol', seats: 2, cargo_capacity: 60,  max_takeoff_weight: 500,  fuel_capacity: 90,  fuel_burn_base: 2.2, fuel_type: 'jet',     altitude_ceiling: 3, cruise_speed: 3, handling: -1, hull_hp: 45, hardpoints: 4, noise: 3, price_buy: 90000, price_rent_hourly: 1500, data: { feel: 'Combat craft; fast, armed, loud — everyone hears it coming.' } },
  { id: 'ac_carcass',   name: 'Carcass',   class: 'wreck',      takeoff_mode: 'stol', seats: 1, cargo_capacity: 0,   max_takeoff_weight: 300,  fuel_capacity: 30,  fuel_burn_base: 2.0, fuel_type: 'biofuel', altitude_ceiling: 1, cruise_speed: 1, handling: 4,  hull_hp: 10, hardpoints: 0, noise: 2, price_buy: 400,   price_rent_hourly: 0,    data: { feel: 'Found downed; rebuild rolls a random type.' } },
];
  ...
  await query(
    `INSERT INTO aircraft_types
     ...
     ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name,class=EXCLUDED.class,takeoff_mode=EXCLUDED.takeoff_mode,seats=EXCLUDED.seats,
       ...
       price_rent_hourly=EXCLUDED.price_rent_hourly,data=EXCLUDED.data`,
    [t.id, t.name, t.class, t.takeoff_mode, t.seats, t.cargo_capacity, t.max_takeoff_weight, t.fuel_capacity, t.fuel_burn_base,
     t.fuel_type, t.altitude_ceiling, t.cruise_speed, t.handling, t.hull_hp, t.hardpoints, t.noise, t.price_buy, t.price_rent_hourly, JSON.stringify(t.data)]
  );
}

// ── The six airfields (flag existing thematically-fitting zones) ──────────────
const FIELDS = [
  { zone: 'zone_threshold',        id: 'af_helipad',    name: 'Threshold Helipad',   fuels: ['avgas', 'jet'],            dealer: false, charter: true  },
  { zone: 'zone_outskirts',        id: 'af_regional',   name: 'Coldwater Regional',  fuels: ['avgas', 'jet', 'biofuel'], dealer: true,  charter: true  },
  { zone: 'zone_yard_marshalling', id: 'af_marshalling',name: 'Marshalling Field',   fuels: ['avgas', 'jet'],            dealer: false, charter: true  },
  { zone: 'zone_slag_gate',        id: 'af_slagworks',  name: 'Slagworks Strip',     fuels: ['avgas', 'biofuel'],        dealer: false, charter: true  },
  { zone: 'zone_waste_scald',      id: 'af_redline',    name: 'Redline Airstrip',    fuels: ['biofuel'],                 dealer: false, charter: false },
  { zone: 'zone_dock_slip',        id: 'af_bay',        name: "Smuggler's Slip Pad", fuels: ['avgas', 'biofuel'],        dealer: false, charter: true  },
];
...
for (const f of FIELDS) {
  ...
  await query(
    `UPDATE zones SET flags = COALESCE(flags,'{}'::jsonb) || jsonb_build_object(
       ...
       'airfield_fuels', $3::jsonb, 'airfield_dealer', $4::boolean, 'airfield_charter', $5::boolean)
     WHERE id=$6`,
    [f.id, f.name, JSON.stringify(f.fuels), f.dealer, f.charter, f.zone]
  );
  console.log(`Airfield ${f.name} @ ${f.zone} (fuels ${f.fuels.join('/')}${f.dealer ? ', dealer' : ''}${f.charter ? ', charter' : ''}).`);
}
