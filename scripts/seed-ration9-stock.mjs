...
// so Ration Nine's Ironchill fridge/freezer are populated immediately instead
// of waiting for the next 24h dailyMaintenance() tick.
...
const { rows: counts } = await query(
  `SELECT container_id, item_id, COUNT(*)::int AS n FROM player_inventory
    WHERE container_id IN ('furn_ration9_fridge','furn_ration9_freezer')
